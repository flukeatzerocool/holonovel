...
spec-engineering loop. All 56 spec features inventoried and priority-ranked
...
1. State model & 6 property groups — NPC, Scene, Countdown, Lore,
   Enrichment, Adventure tiers as architecturally coupled state containers
   (§7.7, REQ-075/076/073/083/080/079). Score: 57 (freq=5, crit=5,
   cplx=5, coupling=5, maturity=3, coverage=3). Touches every tool; the
   architectural spine of the server.
2. Server-side hat gating & access — hat activation, set_hat, enforced
...
8. Spec-driven updates — gap audit across tool catalog, resource map, prompt
   ...
   Delta class wrong = silent regressions.
9. Snapshots, undo, redo — LIFO snapshot stack, min-10 undo depth, redo stack
...
10. Error taxonomy & response contract — [OK]/[NEED_INPUT]/[PARTIAL]/[ERROR]/
    ...
    misrouting.
11. Extraction & confidence — HIGH/MEDIUM/LOW labels, per-section aggregation,
    ...
    truth.
12. Verification workflows — G0 (intake integrity + MCP conformance), G2
...
13. Character creation workflow — step-by-step (sequential NEED_INPUT
...
14. Handoff artifacts & verification — 4 artifacts (RULESET_MODEL.md,
...
15. Dynamic lore — keyword-triggered entries, hat-filtered, priority ordering,
    ...
    drift.
16. Countdowns — round (auto-decrement on combat round) and narrative (GM
...
17. Named-NPC state — create, update, remove, stat block reference from
...
18. Determinism & RNG — single PRNG, seedable via TTRPG_SEED, per-call seeds,
...
19. Independent verification — cold checkout, Phase 1 blind re-execution (G0
...
20. Enrichment workflow — 6 output modules (voice examples, prompt ordering,
...
21. Action suggestions — suggest_actions mapping natural-language intent to
    ...
    coverage=3).
22. Scene-state ledger — scene://current, scene://history, structured fields
    ...
    coverage=3).
23. Post-write verification — heading structure check, path corruption scan,
...
24. Intake workflow — Q0 (workflow selection), build-mode profiles
...
25. Safety & containment — no runtime network access (REQ-051), path
...
26. Entity personality fields — description, voice, background, goals,
...
27. Audit log — append-only, tamper-evident (chained hashes), records every
    ...
    coupling=4, maturity=3, coverage=2).
28. Hat briefing composition — ordered section groups (foundations → anti-slop
...
29. Adventure modules & generation — load_adventure, indexed adventure content
...
30. Roll transparency — full calculation path (dice notation, individual faces,
...
31. Player signals & briefing — player_signal (pace/difficulty/tone/focus/
    ...
    maturity=4, coverage=3).
32. Session recap — structured summary (timespan, entity states, confrontations,
    ...
    coverage=2).
33. Multi-entity support — multiple entities per Novel, roster holds multiple,
...
34. Source conversion — PDF/HTML/web scrape → Markdown per Appendix G,
...
35. Novel & lorebook interchange — export_novel/export_lorebook (json/markdown),
...
36. Scene type tagging — set_scene_type (combat/social/exploration/neutral,
    ...
    coupling=3, maturity=4, coverage=2).
37. Narrative directive — set_narrative_directive with labeled directives
...
38. Canonical lookups — lookup_<category> tools accepting canonical names and
    ...
    coupling=2, maturity=3, coverage=3).
39. Tools surface — minimum categories (character creation, condition
    ...
    coverage=3).
40. Prompts — dynamic composition from live index, state snapshot, registry
    ...
    maturity=4, coverage=3).
41. Resources — full URI catalog (ruleset://, entities://, entity://, audit://,
    ...
    coverage=3).
42. Decision workflows — respond(decision, option), kebab-cased ≤25 options
...
43. spec_health reporting — confidence scores (per-file and overall),
    ...
    crit=3, cplx=2, coupling=2, maturity=4, coverage=3).
44. Help & tool discovery — help tool with optional query, no-query returns
...
45. Audit compression — compress_audit(max_entries) returns formatted prompt of
...
46. Macro system — {{<path>}} token expansion in tool output, resource text,
...
47. Truncation & result counting — output:// pointers for output exceeding
    ...
    coverage=2).
48. Verbose output — every ruleset-defined field returned (not summaries),
...
49. Parameter canon validation — bounded-domain tool parameters (skill names,
...
50. Performance benchmarks — cold-start ≤5s (Standard tier, tiered thresholds
...
51. Anti-slop guidance — concrete forbidden narrative patterns with corrected
    ...
    coverage=2).
52. Enrichment reversion — revert_enrichment removes all six enrichment modules,
...
53. Prompt section ordering — set_briefing_order(sections) accepts ordered
    ...
    coverage=2).
54. Source immutability & drift detection — ruleset Markdown hashed at intake
    ...
    maturity=4, coverage=2).
55. Build fingerprint & version coordination — spec version (CalVer), ruleset
    ...
    crit=2, cplx=1, coupling=1, maturity=4, coverage=2).
56. Runtime conventions — anchor derivation (lowercase, strip punctuation,
