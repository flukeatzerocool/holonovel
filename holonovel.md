...
signal is recorded in the audit log and surfaced in `hat_briefing` as current player
...
pure-resolution (idempotent, no state mutation). Results are hat-filtered: GM-only
...
identified during discovery. The curated set and its derivation — which ruleset
sections define each category — are recorded in RULESET_MODEL.md. Coverage below
80% across the curated set is recorded as a suggestion-coverage finding in
DECISIONS.md (5) with the uncovered categories named. This is a build-time
audit; suggestion mappings do not change at runtime. _Check:_ T117.
