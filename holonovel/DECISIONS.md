...
| Reused | server, extraction, tooling |
| Verification | assemble + check:fast 0 errors; `validate:sdd` 0 errors; 6 DP-3-accepted warnings remain — three metric-name false positives whose "MUST" is a named §6.5 convergence metric (`MUST-coverage` in REQ-208a, `MUST coverage` in REQ-244b, `MUST-action coverage` in REQ-383a), one genuine RFC-2119 `MUST` verb (REQ-162b, "tools MUST pass the Pattern Buffer"), and two narrative-prose grade readings (reading guide "Phased artifact model", mission "Ruleset package model") that cannot reach the grade-12 bar without removing required REQ citations or canonical Spec-Kit phase names |
