...
async function main() {
  ...
  await test("T210/T47/REQ-003 + REQ-060 + REQ-061/REQ-280: ruleset lookup full entry, source quoting, roll transparency", async () => {
    seedRuleset();
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w1", ruleset: "wave1test" });
    ...
    const nl = await call(p, "manage_ruleset", { action: "search",  query: "goblin" });
  ...
  await test("T461/REQ-064 + T223/REQ-070 + T26/REQ-071: briefing orientation layer", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w1b" });
  ...
  await test("T123/REQ-118: prompt budget truncation with never-truncated elements", async () => {
    ...
    await call(p, "manage_novel", { action: "create",  name: "w1e" });
    const brief = await proto(p, "prompts/get", { name: "badge_briefing" });
  ...
  await test("T223/REQ-184 + REQ-070: guidance anti-slop resource badge-filtered", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w1c" });
  ...
  await test("T13/REQ-004 + T221/REQ-179: truncation with output:// pointer retrieval", async () => {
    ...
    await call(p, "manage_novel", { action: "create",  name: "w1d", ruleset: "wave1test" });
    await call(p, "set_badge", { badge: "game_master" });
    const s = await call(p, "manage_ruleset", { action: "search",  query: "fireball" });
  ...
  await test("T48/REQ-061 + T329/REQ-280: source quoting and ruleset-free null source_anchor", async () => {
    seedRuleset();
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w1f" });
    await call(p, "set_badge", { badge: "game_master" });
    // Ruleset-free novel with installed rulesets: search reports no binding.
    const rf = await call(p, "manage_ruleset", { action: "search",  query: "nothing" });
    assertContains(rf, "No ruleset bound", "REQ-280 ruleset-free no source anchor path");
    await call(p, "manage_novel", { action: "create",  name: "w1g", ruleset: "wave1test" });
    const g = await call(p, "manage_ruleset", { action: "search",  query: "fireball" });
  ...
  await test("T105/REQ-106 + T486/REQ-413 + T487/REQ-414 + T488/REQ-415: spec_health surfaces", async () => {
    ...
    await call(p, "manage_novel", { action: "create",  name: "w1h" });
    ...
    if (typeof health.tool_parameter_counts?.manage_character !== "number") throw new Error("REQ-408 param counts missing");
    ...
    if ((health.tool_required_param_counts?.manage_character ?? 0) > health.parameter_ceiling) {
      throw new Error(`REQ-408 per-action ceiling exceeded: character requires ${health.tool_required_param_counts?.manage_character} > ${health.parameter_ceiling}`);
    }
  ...
  await test("T39/REQ-057 + T39a/REQ-059 + T39b/REQ-183 + T115/REQ-112: lookup NOT_FOUND enumeration and cross-refs", async () => {
    seedRuleset();
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w1i", ruleset: "wave1test" });
    await call(p, "set_badge", { badge: "game_master" });
  ...
  await test("T124/REQ-078: session zero has all 8 sections", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w1j" });
  ...
  await test("T152/REQ-138 + T153/REQ-139 + T204/REQ-169 + T289/REQ-269: spec_health reporting surfaces", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w1k" });
    const health = JSON.parse(await call(p, "manage_session", { action: "health" }));
  ...
  await test("T9/REQ-031 + T147/REQ-133: badge activation and forbidden-call audit", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w3a" });
    const s = await call(p, "set_badge", { badge: "game_master" });
    assertContains(s, "[OK]", "REQ-031 badge activation");
    await call(p, "set_badge", { badge: "player" });
    const fb = await call(p, "manage_npc", { action: "create",  name: "Denied" });
  ...
  await test("T257/REQ-216: GM-only generation table filtered under Player", async () => {
    seedRuleset();
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w3b", ruleset: "wave1test" });
    await call(p, "set_badge", { badge: "player" });
    const roll = await call(p, "manage_ruleset", { action: "roll",  table: "secret_table" });
    ...
    await call(p, "set_badge", { badge: "game_master" });
    const gmRoll = await call(p, "manage_ruleset", { action: "roll",  table: "secret_table" });
  ...
  await test("T262/REQ-220 + T265/REQ-223: POV directive and pov mode control", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w3c" });
    await call(p, "set_badge", { badge: "player" });
    await call(p, "manage_character", { action: "create",  name: "PovChar" });
    await call(p, "manage_character", { action: "set_active",  entity_id: "character_01", pov: "omniscient" });
    ...
    await call(p, "manage_character", { action: "set_active",  entity_id: "character_01", pov: "character" });
  ...
  await test("T348/REQ-304: TTRPG_AI_ROLE locks orientation", async () => {
    ...
    await call(p, "manage_novel", { action: "create",  name: "w3d" });
    await call(p, "set_badge", { badge: "game_master" });
    const brief = await proto(p, "prompts/get", { name: "badge_briefing" });
  ...
  await test("T330/REQ-281: narrative_threads section renders in briefing", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w3f" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_countdown", { action: "set",  name: "Doom", ticks: 3 });
    await call(p, "manage_npc", { action: "create",  name: "Grumpy", disposition: "hostile" });
    const brief = await proto(p, "prompts/get", { name: "badge_briefing" });
  ...
  await test("T216/REQ-176 + T217/REQ-177 + T219/REQ-178: entity/roster removal and listing", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w4a" });
    await call(p, "manage_character", { action: "create",  name: "R1", stage_to_roster: true });
    await call(p, "set_badge", { badge: "game_master" });
    const lst = await call(p, "manage_character", { action: "roster_list" });
    assertContains(lst, "character_01", "REQ-178 roster listing");
    const rm = await call(p, "manage_character", { action: "roster_remove",  roster_id: "character_01" });
    assertContains(rm, "[OK]", "REQ-177 roster removal");
    const rm2 = await call(p, "manage_character", { action: "roster_remove",  roster_id: "character_01" });
    assertContains(rm2, "[NOT_FOUND]", "REQ-177 NOT_FOUND on absent");
    const entRm = await call(p, "manage_character", { action: "remove",  entity_id: "character_01" });
  ...
  await test("T258/REQ-217: condition tools with validation and warnings", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w4b" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "Cond" });
    const a = await call(p, "manage_condition", { action: "apply",  entity_id: "character_01", condition: "prone" });
    assertContains(a, "[OK]", "REQ-217 apply");
    const dup = await call(p, "manage_condition", { action: "apply",  entity_id: "character_01", condition: "prone" });
    assertContains(dup, "[WARNING]", "REQ-217 duplicate warning");
    const bad = await call(p, "manage_condition", { action: "apply",  entity_id: "character_01", condition: "not_a_condition" });
    assertContains(bad, "[INVALID_INPUT]", "REQ-217 unknown condition");
    const rem = await call(p, "manage_condition", { action: "remove",  entity_id: "character_01", condition: "prone" });
  ...
  await test("T275/REQ-237 + T328/REQ-279: session segmentation and narrative orientation", async () => {
    ...
    await call(p, "manage_novel", { action: "create",  name: "w4c" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_story", { action: "record",  type: "decision", entry: "The party weighs the two doors." });
  ...
  await test("T352/REQ-308 + T374/T377/REQ-330: knowledge gating by presence and exploration", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w4d" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "Explorer" });
    await call(p, "manage_world", { action: "create_room",  name: "Entrance", description: "d" });
    await call(p, "manage_world", { action: "create_room",  name: "Guard Room", description: "g" });
    await call(p, "manage_world", { action: "create_exit",  direction: "north", room_a: "Entrance", room_b: "Guard Room" });
    const nav = await call(p, "run_command", { command: "go north" });
  ...
  await test("T369/REQ-322: vow-countdown coupling", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w4e" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_vow", { action: "set",  name: "Find Crown", description: "Recover the Crown", parties: ["pc_1"], difficulty: "dangerous", scope: "shared" });
    const brief = await proto(p, "prompts/get", { name: "badge_briefing" });
    assertContains(brief, "Vow countdown suggestion", "REQ-322 suggestion in threads");
    await call(p, "respond_decision", { decision: "accept", option: "accept" });
    const brief2 = await proto(p, "prompts/get", { name: "badge_briefing" });
    assertContains(brief2, "vow:Find Crown", "REQ-322 linked countdown created");
    await call(p, "manage_vow", { action: "milestone",  vow_name: "Find Crown" });
    passed++;
    await kill(p);
  });
  ...
  await test("T80/REQ-094 + T340/REQ-295: lorebook round-trip and genre-filtered generation", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w4f" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_lore", { action: "set",  key: "city", content: "The city is old.", triggers: ["city"], badge_scope: "shared" });
    const exp = await call(p, "manage_lore", { action: "export" });
    const imp = await call(p, "manage_lore", { action: "import",  data: exp, mode: "replace" });
    assertContains(imp, "[OK]", "REQ-094 import replace");
    const exp2 = await call(p, "manage_lore", { action: "export" });
    ...
    void playerBlocked;
    const expP = await call(p, "manage_lore", { action: "export" });
  ...
  await test("T373/T376/REQ-329: countdown on_room_enter trigger advances", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w4g" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "Wanderer" });
    await call(p, "manage_world", { action: "create_room",  name: "Hall", description: "h" });
    await call(p, "manage_world", { action: "create_room",  name: "guard_room", description: "g" });
    await call(p, "manage_world", { action: "create_exit",  direction: "east", room_a: "Hall", room_b: "guard_room" });
    await call(p, "manage_countdown", { action: "set",  name: "ambush", ticks: 2, type: "narrative", triggers: ["on_room_enter(guard_room)"] });
    const nav = await call(p, "run_command", { command: "go east" });
  ...
  await test("T126/REQ-119 + T127/REQ-120 + T128/REQ-121: NPC stat reference, rendering, resources", async () => {
    seedRuleset();
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w5a", ruleset: "wave1test" });
    await call(p, "set_badge", { badge: "game_master" });
    const created = await call(p, "manage_npc", { action: "create",  name: "Goblin", ruleset_reference: "Goblin" });
    ...
    const bad = await call(p, "manage_npc", { action: "create",  name: "X", ruleset_reference: "NoSuchThing" });
  ...
  await test("T129/REQ-122 + T191/REQ-156: NPC personality fields and description field", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w5b" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_npc", { action: "create",  name: "Guard", description: "Tall" });
    const npcs = JSON.parse(await proto(p, "resources/read", { uri: "npcs://" }));
    const id = Object.keys(npcs)[0];
    await call(p, "manage_character", { action: "personality",  entity_id: id, description: "Suspicious" });
  ...
  await test("T356/REQ-311: NPC memory updates on combat engagement", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w5c" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "Fighter" });
    await call(p, "manage_npc", { action: "create",  name: "Thug" });
    const npcs = JSON.parse(await proto(p, "resources/read", { uri: "npcs://" }));
    const id = Object.keys(npcs)[0];
    await call(p, "manage_combat", { action: "init",  participants: ["character_01", id] });
  ...
  await test("T130/REQ-123 + T191/REQ-156 + T200/REQ-165 + T201/REQ-166 + T202/REQ-167: NPC stat fields, personality rendering", async () => {
    ...
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_npc", { action: "create",  name: "Smith", description: "A smith" });
    const npcs = JSON.parse(await proto(p, "resources/read", { uri: "npcs://" }));
    const id = Object.keys(npcs)[0];
    await call(p, "manage_character", { action: "personality",  entity_id: id, voice: "gruff" });
    await call(p, "manage_character", { action: "voice",  entity_id: id, examples: [{ context: "greeting", dialogue: "What do you need?" }] });
  ...
  await test("T203/REQ-168 + T332/REQ-282: audit resource and NPC voice directive", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w5e" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "PC" });
    const aud = await proto(p, "resources/read", { uri: "audit://novel" });
    assertContains(aud, "create_novel", "REQ-168 audit resource");
    await call(p, "manage_npc", { action: "create",  name: "Smith" });
    const npcs = JSON.parse(await proto(p, "resources/read", { uri: "npcs://" }));
    const id = Object.keys(npcs)[0];
    await call(p, "manage_npc", { action: "update",  npc_id: id, location: "smithy" });
    await call(p, "manage_character", { action: "voice",  entity_id: id, examples: [{ context: "hi", dialogue: "Gruff hello." }] });
    await call(p, "manage_scene", { action: "set",  description: "forge", location: "smithy" });
    const brief = await proto(p, "prompts/get", { name: "badge_briefing" });
  ...
  await test("T146/REQ-132 + T285/REQ-248 + T286/REQ-249: generated adventure lifecycle and resources", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w6a" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_adventure", { action: "generate",  premise: "A haunted station" });
    ...
    await call(p, "manage_scene", { action: "set",  description: "scene", adventure_scene: "hall" });
    passed++;
    await kill(p);
  });
  ...
  await test("T207/REQ-170 + T338/REQ-292: adventure catalog and discovery surface", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w6b" });
    const cat = await call(p, "manage_adventure", { action: "list" });
  ...
  await test("T284/REQ-250 + T312/REQ-252: adventure waypoint and fast-forward", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w6c" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_adventure", { action: "generate",  premise: "A tower" });
    await call(p, "manage_countdown", { action: "set",  name: "ritual", ticks: 3, type: "narrative" });
    await call(p, "manage_scene", { action: "set",  description: "foyer", fast_forward: { interval: "three days of travel" } });
    const brief = await proto(p, "prompts/get", { name: "badge_briefing" });
    void brief;
    const aud = await proto(p, "resources/read", { uri: "audit://novel" });
  ...
  await test("T305/REQ-229: load_adventure synthesis augmentation section", async () => {
    ...
    await call(p, "manage_novel", { action: "create",  name: "w6d" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_synthesis", { action: "run" });
    const load = await call(p, "manage_adventure", { action: "load",  slug: "test-tower" });
  ...
  await test("T246/REQ-203 + T247/REQ-204 + T248/REQ-205: combat init guard, participant validation, mid-combat changes", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w7a" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "Hero" });
    await call(p, "manage_npc", { action: "create",  name: "Goblin" });
    const g = await call(p, "manage_combat", { action: "init",  participants: ["nonexistent"] });
    assertContains(g, "[NOT_FOUND]", "REQ-204 participant validation");
    const c1 = await call(p, "manage_combat", { action: "init",  participants: ["character_01", "npc_"] });
    void c1;
    const npcs = JSON.parse(await proto(p, "resources/read", { uri: "npcs://" }));
    const npcId = Object.keys(npcs)[0];
    await call(p, "manage_combat", { action: "init",  participants: ["character_01", npcId] });
    const again = await call(p, "manage_combat", { action: "init",  participants: ["character_01", npcId] });
    assertContains(again, "[STATE_CONFLICT]", "REQ-203 combat-init guard");
    const add = await call(p, "manage_combat", { action: "add_participant",  participant_id: "nonexistent2" });
  ...
  await test("T249/REQ-206: combat-round condition expiry", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w7b" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "Fighter" });
    await call(p, "manage_condition", { action: "apply",  entity_id: "character_01", condition: "prone", rounds: 1 });
    await call(p, "manage_combat", { action: "init",  participants: ["character_01"] });
    await call(p, "manage_combat", { action: "advance" });
    await call(p, "manage_combat", { action: "advance" });
    const sheet = await call(p, "manage_character", { action: "sheet",  entity_id: "character_01" });
  ...
  await test("T263/REQ-221: combat-navigation interaction blocks movement", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w7c" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "W" });
    await call(p, "manage_world", { action: "create_room",  name: "A", description: "a" });
    await call(p, "manage_world", { action: "create_room",  name: "B", description: "b" });
    await call(p, "manage_world", { action: "create_exit",  direction: "north", room_a: "A", room_b: "B" });
    await call(p, "manage_combat", { action: "init",  participants: ["character_01"] });
    const nav = await call(p, "run_command", { command: "go north" });
    assertContains(nav, "[STATE_CONFLICT]", "REQ-221 navigation blocked in combat");
    const look = await call(p, "run_command", { command: "look" });
    assertNotContains(look, "[ERROR]", "REQ-221 inspection allowed");
    await call(p, "manage_combat", { action: "end" });
    const nav2 = await call(p, "run_command", { command: "go north" });
  ...
  await test("T311/REQ-251: generation intent guard warns on harm/power-inversion", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w7d" });
    await call(p, "set_badge", { badge: "game_master" });
    const warn = await call(p, "manage_adventure", { action: "generate",  premise: "create an adversary capable of defeating Hero" });
    assertContains(warn, "[WARNING]", "REQ-251 generation guard warns");
    const force = await call(p, "manage_adventure", { action: "generate",  premise: "!force create an adversary capable of defeating Hero" });
  ...
  await test("T313/REQ-253: terse output verbosity", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w7e" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_session", { action: "verbosity",  mode: "terse" });
    const health = JSON.parse(await call(p, "manage_session", { action: "health" }));
  ...
  await test("T192/REQ-157: combat determinism with per-call seed", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w7f" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_combat", { action: "init",  participants: [], dangers: [{ name: "goblin" }, { name: "orc" }], seed: "42" });
    await call(p, "manage_combat", { action: "end" });
    await call(p, "manage_combat", { action: "init",  participants: [], dangers: [{ name: "goblin" }, { name: "orc" }], seed: "42" });
    const aud = await proto(p, "resources/read", { uri: "audit://novel" });
  ...
  await test("T20/T42/REQ-054: adversarial input stored and echoed verbatim", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w8a" });
    await call(p, "set_badge", { badge: "game_master" });
    const evil = "'); DROP TABLE novels;--";
    const s = await call(p, "manage_scene", { action: "set",  description: evil });
  ...
  await test("T357/REQ-312 + T490/REQ-417: narration validation gate and startup probes", async () => {
    ...
    await call(p, "manage_novel", { action: "create",  name: "w8b" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_npc", { action: "create",  name: "Corpse", stats: { hp: 0 } });
    const h1 = JSON.parse(await call(p, "manage_session", { action: "health" }));
  ...
  await test("T70/REQ-086: compress_audit format and INVALID_INPUT", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w9a" });
    await call(p, "set_badge", { badge: "game_master" });
    const ca = await call(p, "manage_session", { action: "compress",  max_entries: 5 });
    assertContains(ca, "Compressed audit log", "REQ-086 header line");
    const bad = await call(p, "manage_session", { action: "compress",  max_entries: 0 });
  ...
  await test("T476/REQ-087 + T119/REQ-115: scene type tagging and action pattern toggle", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w9b" });
    await call(p, "set_badge", { badge: "game_master" });
    const st = await call(p, "manage_scene", { action: "set",  description: "social scene", scene_type: "social" });
    assertContains(st, "[OK]", "REQ-087 scene type");
    const tg = await call(p, "manage_synthesis", { action: "toggle_action" });
  ...
  await test("T300/REQ-185 + T225/REQ-186: section token vocabulary and discoverability", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w9c" });
    await call(p, "set_badge", { badge: "game_master" });
    const health = JSON.parse(await call(p, "manage_session", { action: "health" }));
    if (!Array.isArray(health.section_tokens) || health.section_tokens.length === 0) throw new Error("REQ-185/186 section_tokens missing");
    const bad = await call(p, "manage_session", { action: "briefing_order",  sections: ["not_a_token"] });
  ...
  await test("T299/REQ-155: sticky counter decay", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w9d" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_lore", { action: "set",  key: "k", content: "c", triggers: ["fire"], sticky: 2, badge_scope: "shared" });
    await call(p, "manage_scene", { action: "set",  description: "the fire burns" });
    await call(p, "manage_scene", { action: "set",  description: "quiet now" });
    await call(p, "manage_scene", { action: "set",  description: "quiet now" });
    const brief = await proto(p, "prompts/get", { name: "badge_briefing" });
  ...
  await test("T307/REQ-231 + T322/REQ-263: per-module toggle and auto-trigger reporting", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w9e" });
    await call(p, "set_badge", { badge: "game_master" });
    const bad = await call(p, "manage_synthesis", { action: "toggle",  module: "nope", enabled: false });
  ...
  await test("T321/REQ-262 + T323/REQ-264 + T326/REQ-265: synthesize tool and briefing presence", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w9f" });
    await call(p, "set_badge", { badge: "game_master" });
    const s = await call(p, "manage_synthesis", { action: "run" });
    assertContains(s, "[OK]", "REQ-262 synthesize runs");
    const s2 = await call(p, "manage_synthesis", { action: "run" });
  ...
  await test("T355/REQ-310: campaign memory counts", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w9g" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_npc", { action: "create",  name: "MemNpc" });
    const h = JSON.parse(await call(p, "manage_session", { action: "health" }));
  ...
  await test("T375/REQ-328 + T378/REQ-331: lore-world and story-world coupling", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w9h" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_lore", { action: "set",  key: "altar", content: "The altar hums", triggers: ["altar"], world_target: "altar_01" });
    const s = await call(p, "manage_story", { action: "record",  type: "moment", entry: "Found the door" });
  ...
  await test("T302/REQ-226: narrative voice profiles resource", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w9i" });
  ...
  await test("T240/REQ-197 + T333/REQ-283: description modes and verb coverage", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w10a" });
    await call(p, "set_badge", { badge: "game_master" });
    const b = await call(p, "run_command", { command: "brief" });
    assertContains(b, "[OK]", "REQ-197 brief mode");
    const v = await call(p, "run_command", { command: "verbs" });
  ...
  await test("T370/REQ-326 + T371/REQ-327: scene-world and NPC-world coupling", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w10b" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_world", { action: "create_room",  name: "Throne Room", description: "golden" });
    await call(p, "manage_npc", { action: "create",  name: "Blacksmith", location: "Throne Room" });
    await call(p, "manage_scene", { action: "set",  description: "The throne room", location: "Throne Room" });
    const cur = await proto(p, "resources/read", { uri: "scene://current" });
    assertContains(cur, "Throne Room", "REQ-326 scene-world coupling room match");
    const npcs = JSON.parse(await proto(p, "resources/read", { uri: "npcs://" }));
    const id = Object.keys(npcs)[0];
    const upd = await call(p, "manage_npc", { action: "update",  npc_id: id, location: "Inn" });
  ...
  await test("T419/REQ-368: countdown world_effect fires and applies", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w10c" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_world", { action: "create_room",  name: "cellar", description: "dry" });
    await call(p, "manage_countdown", { action: "set",  name: "flood", ticks: 1, type: "narrative", world_effect: { type: "describe", target: "cellar", value: "Knee-deep water" } });
    await call(p, "manage_countdown", { action: "advance",  name: "flood" });
    const cur = await proto(p, "resources/read", { uri: "scene://current" });
    void cur;
  ...
  await test("T372/REQ-325: constraint override catalog", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w10d" });
    await call(p, "set_badge", { badge: "game_master" });
  ...
  await test("T354/REQ-284: implicit action hints on locked open", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w10e" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "W" });
    await call(p, "manage_world", { action: "create_room",  name: "Hall", description: "h" });
    await call(p, "manage_world", { action: "create_thing",  name: "chest", description: "a chest", openable: true, lockable: true, locked: true, fixed: true, location: "Hall" });
    const noKey = await call(p, "run_command", { command: "open chest" });
    assertContains(noKey, "locked", "REQ-284 locked message");
    await call(p, "manage_world", { action: "create_thing",  name: "iron key", description: "k", location: "Hall" });
    const hint = await call(p, "run_command", { command: "open chest" });
  ...
  await test("T418/REQ-367: property propagation across containment", async () => {
    ...
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "W2" });
    await call(p, "manage_world", { action: "create_room",  name: "Dark", description: "d" });
    await call(p, "manage_world", { action: "create_thing",  name: "glass jar", description: "j", openable: true, fixed: true, transparent: true, location: "Dark" });
    await call(p, "manage_world", { action: "create_thing",  name: "lantern", description: "l", lit: true, switched_on: true, location: "glass jar", location_type: "container" });
    const look = await call(p, "run_command", { command: "look" });
  ...
  await test("T460/REQ-393 + T459/REQ-392 + T456/REQ-391: update preservation, tool budget, scoped listing", async () => {
    seedRuleset();
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w11b", ruleset: "wave1test" });
    const h = JSON.parse(await call(p, "manage_session", { action: "health" }));
  ...
  await test("T32/REQ-056: advancement workflow raises NEED_INPUT per open choice", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w11c" });
    await call(p, "set_badge", { badge: "game_master" });
    const nf = await call(p, "manage_character", { action: "create" });
    assertContains(nf, "[NEED_INPUT]", "REQ-056 workflow raises");
    await call(p, "respond_decision", { decision: "cancel", option: "cancel" });
    passed++;
    await kill(p);
  });
  ...
  await test("T494/REQ-193: pending workflow staleness surfaced in spec_health", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w11d" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create" });
    const h = JSON.parse(await call(p, "manage_session", { action: "health" }));
  ...
  await test("T505/REQ-425: output format catalog uniformity + validation", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w12a" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "Fen", description: "scarred" });
    const entity_id = "character_01";
    // markdown default
    const mdDefault = await call(p, "manage_character", { action: "sheet",  entity_id });
    assertContains(mdDefault, "## Fen", "T505 character_sheet markdown default");
    // json structured
    const j = await call(p, "manage_character", { action: "sheet",  entity_id, format: "json" });
    const parsed = JSON.parse(j);
    if (parsed.name !== "Fen") throw new Error("T505 json name mismatch");
    // html presentational
    const html = await call(p, "manage_character", { action: "sheet",  entity_id, format: "html" });
    assertContains(html, "<h2>", "T505 character_sheet html");
    // ascii
    const ascii = await call(p, "manage_character", { action: "sheet",  entity_id, format: "ascii" });
    assertContains(ascii, "Fen", "T505 character_sheet ascii");
    // unsupported format -> INVALID_INPUT with enumeration
    const bad = await call(p, "manage_character", { action: "sheet",  entity_id, format: "pdf" });
    ...
    await call(p, "manage_codex", { action: "set",  kind: "npc", name: "Blacksmith", content: { ac: 14 } });
    ...
    await call(p, "manage_lore", { action: "set",  key: "treaty", content: "A border treaty." });
  ...
  await test("T506/REQ-425d: ruleset-declared format registry surfaced in spec_health", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w12b" });
    await call(p, "set_badge", { badge: "game_master" });
    const h = JSON.parse(await call(p, "manage_session", { action: "health" }));
  ...
  await test("T507/REQ-426: MCP Apps ui:// surface (negotiated)", async () => {
    ...
    await call(p, "manage_novel", { action: "create",  name: "w12c" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "Fen" });
    const entity_id = "character_01";
    // tool result carries ui:// linkage metadata (REQ-426b)
    const r = await callRaw(p, "manage_character", { action: "sheet",  entity_id });
  ...
  await test("T508/REQ-426c: fallback without negotiation", async () => {
    const p = await boot();
    await call(p, "manage_novel", { action: "create",  name: "w12d" });
    await call(p, "set_badge", { badge: "game_master" });
    await call(p, "manage_character", { action: "create",  name: "Fen" });
    const entity_id = "character_01";
