# AGENTS.md — Inform MCP Server (v2026.08.07)

AI maintainer orientation for the Inform (ruleset-free world-model) MCP server implementation.
...
src/macros.ts           expandMacros — {{entity.name}}, {{scene.current}},
                        ...
                        {{world.thing_count}}.
        ↓
src/enrichment.ts       Enrichment manifest — empty in ruleset-free mode.
                        Additive, inert, idempotent.
        ↓
...
```bash
cd inform && npm run start     # start server
