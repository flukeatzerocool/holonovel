...
- Proofread the resulting spec text against the \`**Prose:**\` block. Verify
  the text was inserted verbatim and reads coherently in context.
- Verify conformance to the spec style guide (Appendix M in holonovel.md):
  the REQ body must state *what* (outcome contract), not *how*. Flag and
  stop if the prose contains:
  * Parameter types (string, number, boolean, integer, Map, Array)
  * \"Default:\" or \"(default ...)\" clauses
  * Enumerated catalogs of >5 tokens in a single sentence
  * Algorithm descriptions, sort orders, or step-by-step procedures
  If any violation is found, report the Change number, the offending text,
  and the rule violated — then stop. Do not apply further changes.
- Run \`npm run check\`. If any check fails, stop immediately and report the
  failing change and the error output.

If all checks pass, report a summary: which changes were applied, any REQ
numbers added, and new test IDs.
