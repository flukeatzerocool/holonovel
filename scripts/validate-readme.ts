...
const toolNames = [
  ...
  "lookup_monster", "lookup_spell", "make_panic_check", "player_signal",
  "remove_condition", "remove_countdown", "remove_lore_entry", "remove_npc",
  "respond", "resume_novel", "roll_damage", "roll_on_table", "roll_save",
  ...
  "set_narrative_directive", "set_badge", "set_personality",
...
function checkHeadingOrder(text: string): Issue[] {
  ...
  const h1 = headings[0];
  if (h1.level !== 1 || !h1.title.startsWith("Holonovel")) {
    issues.push({ line: h1.line, error: true, msg: `First heading must be '# Holonovel', got '# ${h1.title}'` });
  }
...
function checkNearDuplicates(text: string): Issue[] {
  ...
  for (let i = 0; i < sentences.length; i++) {
    const aWords = new Set(sentences[i].toLowerCase().split(/\s+/).filter((w) => w.length > 3));
