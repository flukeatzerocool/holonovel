...
const dnd5ePkg = readJson(join(root, "dnd5e", "package.json"));
ok = check("dnd5e/package.json", dnd5ePkg.version as string, rootVersion) && ok;

const decisionsVersion = grepVersion(
  join(root, "dnd5e", "DECISIONS.md"),
  /\*\*Spec version:\*\*\s*(.+)/m
);
ok = check("dnd5e/DECISIONS.md", decisionsVersion, rootVersion) && ok;

const agentsVersion = grepVersion(
  join(root, "dnd5e", "AGENTS.md"),
  /^# AGENTS\.md.*\(v(.+)\)/m
);
ok = check("dnd5e/AGENTS.md", agentsVersion, rootVersion) && ok;

const readmeVersion = grepVersion(
  join(root, "dnd5e", "README.md"),
  /Holonovel\]\([^)]+\)\s+v(.+)\./
);
ok = check("dnd5e/README.md", readmeVersion, rootVersion) && ok;
...
if (decisionsSpecHash) {
  console.log(`  OK  dnd5e/DECISIONS.md: spec hash present`);
} else {
  console.error(`  FAIL  dnd5e/DECISIONS.md: missing spec hash field`);
  ok = false;
}

const stateContent = readFileSync(join(root, "dnd5e", "src", "state.ts"), "utf-8");
if (stateContent.includes(`SPEC_VERSION: string = JSON.parse`) &&
  ...
  console.log(`  OK  dnd5e/src/state.ts: reads version dynamically from package.json`);
} else {
  console.error(`  FAIL  dnd5e/src/state.ts: must read version dynamically from package.json`);
  ok = false;
}
