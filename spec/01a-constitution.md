...
| World prominence   | Build-time `TTRPG_WORLD_PROMINENCE` setting (REQ-309): `visible` (default), `secondary`, or `prominent`. Controls default surface emphasis of world-model and narrative tools across help, `badge_briefing`, and `suggest_actions`. Skipped in ruleset-free mode. |
