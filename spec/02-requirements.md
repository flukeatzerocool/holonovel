...
property. For `wearable`, the assertion is "It is wearable.". For `readable`, the assertion is "It is
readable.". For `edible`, the assertion is "It is edible.". For `transparent`, the assertion is "It is
transparent.". For `switched_on`, the assertion is "It is switched on.". For `enterable`, the assertion is "It is enterable.". For `climbable`, the assertion is "It is climbable."
...
ruleset is indexed. `ruleset (action: roll)` SHALL return the content-absent message per REQ-214. Navigational couplings (§7.7.1) SHALL be active. The World→Scene, Scene→Lore, and Scene→Countdown couplings function in ruleset-free mode, along with other Advisory/Navigational nature rows.
...
The entity is valid as a combat participant in `combat (action: init)`. It receives a turn in the order and auto-advances with an `[auto]` marker without mechanical effects. Dangers and entities have no hit points, no damage, and no death state. The `command (action: suggest)` tool SHALL include the entity's name and narrative fields in context but SHALL return no mechanical action suggestions. _Check:_ T260.
...
When a countdown fires while the active entity's characters are absent from the scene where the server linked the countdown — entity IDs absent from `characters_present` on all scenes since creation — the server SHALL surface the consequence as a `[discovered]` story journal entry of type `consequence` (REQ-246). The entry SHALL carry the countdown name, the consequence description, the discovery timestamp, and a `discovered` boolean set to `true`. The discovery timestamp records the scene transition when the player's entity next becomes present in a scene linked to the countdown's location.
...
The coupling completeness register (§7.7.1b previous) IS REMOVED — the derivation contract replaces it. *Acceptance criterion:* `npm run validate` reports no pattern rules with zero coupling rows and no coupling rows with mismatched archetype assignments. _Check:_ T421 (amended), T434, T435.
...
Tool names that clash between two rulesets under the prefix scheme (e.g., both rulesets extract a tool named `roll_skill_check`) resolve through the prefix — the tool names are distinct on the registry surface. The mapping of prefix to ruleset slug SHALL be recorded in DECISIONS.md (1) during package construction (§6.4.2) and reported in `spec_health` under a `ruleset_prefix_map` field.
