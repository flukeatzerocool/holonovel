...
The surface further reports story journal entries whose entity IDs overlap the active entities or whose scene anchor matches the current scene (GM only, REQ-246). The surface reports the current autonomy state — all four slider values from `scene (action: autonomy)` (REQ-306) when set. The surface reports campaign memory facts (GM only, REQ-310), world in motion entries (GM only, REQ-233a), and proactive available actions (REQ-084a).
...
The oracle is callable by Player and Game Master badges; no badge SHALL be blocked from consulting it. *Acceptance criterion:* `scene (action: oracle)` succeeds under the Player badge and under the Game Master badge. _Check:_ T481, T337.
...
The `switch on` and `switch off` commands SHALL be registered in the parser command catalog under the `object_interaction` category, standard tier. The `world (action: convert)` tool SHALL recognize the "It is switchable." assertion. The tool SHALL also recognize the "It is switched on." assertion. _Check:_ T361.
...
on the altar reads 'Beware the serpent.'" — the `world (action: convert)` tool SHALL extract
...
Navigation commands: `climb` (climbable targets with associated exits), `enter` (enterable containers or vehicles), and `exit`/`get out` (returns to parent room). Also `sit` (supporters, records sitting) and `stand` (ceases sitting). Meta commands: `again`/`g` (repeats the last command, session-local) and `it`/`them`/`all` (pronoun disambiguation). The `world (action: convert)` tool SHALL associate directional exit adjacency by linking a climbable thing with the exit in the same direction: when "A rope ladder is in the Entrance Chamber.
...
The mode persists for the session — the session discards it on connection close. `command("brief")` and `command("verbose")` are always recognized verbs, even when the world-model tier is empty. `spec_health` SHALL report the current description mode. The `character (action: signal)` interface SHALL accept a `detail` signal with three values. The `terse` value gives room name and exits only, with minimal combat feedback (participant name and result, no full roll transparency). The `normal` value gives balanced output. The `rich` value gives full descriptions, complete roll transparency, and lore trigger notifications.
...
The `ruleset (action: search)` tool SHALL return an empty result set with a clear message indicating no
ruleset is indexed. The `ruleset (action: roll)` tool SHALL return the content-absent message per REQ-214. Navigational couplings (§7.7.1) SHALL be active. The World→Scene, Scene→Lore, and Scene→Countdown couplings function in ruleset-free mode, along with other Advisory/Navigational nature rows.
...
The entity is valid as a combat participant in `combat (action: init)`. The entity receives a turn in the order and auto-advances with an `[auto]` marker without mechanical effects. Dangers and entities have no hit points, no damage, and no death state. The `command (action: suggest)` tool SHALL include the entity's name and narrative fields in context but SHALL return no mechanical action suggestions. _Check:_ T260.
...
The AI narrator resolves the player's spatial intent through `command (action: resolve)` (REQ-323) without exposing parser mechanics. The spatial surface in `badge_briefing` gives the player direct awareness of surroundings, without requiring the AI narrator to describe every detail explicitly.
...
WHEN the GM reveals a secret to an entity via `lore (action: reveal)` (REQ-234) AND a countdown exists whose `scope` or `direction` text references the secret's key, THE server SHALL surface an advisory in the `narrative_threads` briefing section (REQ-281) suggesting the countdown be advanced. The advisory SHALL carry the secret's key, the countdown name, and a prompt for the GM to advance or ignore.
...
WHEN a relationship type changes from `ally` to `rival` or `hostile` via `relationship (action: set)` (REQ-236) AND a countdown exists whose `scope` or `direction` text references either entity in the relationship, THE server SHALL surface an advisory in the `narrative_threads` briefing section (REQ-281). The advisory suggests advancing the countdown or creating a new countdown to represent the fallout. The advisory SHALL carry both entity names, the relationship change, and the matching countdown name. A navigational coupling — the server suggests; the GM decides. *Acceptance criterion:* Create countdown with `scope="npc_guard"`.
...
WHEN the GM creates or updates a lore entry via `lore (action: set)` or `lore (action: update)` (REQ-083), the server SHALL check the lore entry's `triggers`. When those triggers include temporal urgency keywords and no countdown matches the lore entry's `key`, THE server SHALL surface a countdown-creation advisory in the `narrative_threads` briefing section (REQ-281). The urgency keywords are "imminent", "approaching", "deadline", "ticking", and "countdown". The advisory SHALL carry the lore entry key, the matched urgency trigger, and a prompt for the GM to create or ignore.
...
The `novel (action: create)` tool SHALL accept a `ruleset` parameter — a ruleset slug matching a prefix in the server's `ruleset_prefix_map`. The Novel's `ruleset` field SHALL stay immutable for the Novel's lifetime except the single audited migration path (REQ-380c). The `novel (action: resume)` tool SHALL restore the bound ruleset from the Novel's persisted state. The `novel (action: clone)` tool SHALL preserve the source Novel's ruleset. The `novel (action: switch)` tool, to a Novel bound to a different ruleset, SHALL activate that Novel's ruleset scope — the ruleset-derived tool surface changes to match. The `novel (action: create)` tool with a ruleset slug absent from the server's `ruleset_prefix_map` returns `[ERROR] [INVALID_INPUT]` with valid rulesets enumerated.

**REQ-380b — Novel ruleset binding (Part b).**
A conformant server with exactly one ruleset MAY accept `novel (action: create)` without the `ruleset` parameter, defaulting to that single ruleset. The default preserves backward compatibility with single-ruleset servers built before multi-ruleset support. *Acceptance criterion:* `novel (action: create, "greyhawk", ruleset="dnd5e")` succeeds and the Novel's `ruleset` field is `"dnd5e"`. Subsequent calls to `dnd5e_roll_skill_check` succeed against this Novel. `novel (action: create, "absalom", ruleset="starfinder")` succeeds with `ruleset: "starfinder"`.
...
Every codex entry SHALL record a `ruleset` field — a slug matching an entry in the server's `ruleset_prefix_map`, or `null` for ruleset-agnostic entries (rooms, scenes, generic NPCs without mechanical stats, lore entries). The `codex (action: set)` tool SHALL default the `ruleset` field to the active Novel's ruleset scope when one is active, or leave it `null` when no Novel is active (the caller MAY override). The `codex (action: capture)` tool SHALL default `ruleset` to the source Novel's ruleset scope. The `codex (action: list)` tool SHALL support a `ruleset` filter parameter, returning only entries whose `ruleset` matches or is `null`. The `codex (action: import)` tool, into a Novel, SHALL show only entries whose `ruleset` matches the Novel's ruleset or is `null`.
...
The `ruleset (action: install)` tool SHALL validate slug uniqueness and package-format compatibility (REQ-420) before activation. The `ruleset (action: remove)` tool SHALL deregister the package's tools, resources, and prompts and SHALL refuse while any active Novel binds to its slug. The `ruleset (action: list)` tool SHALL report each installed package with installed-versus-loaded state. All three are Game Master or Editor operations and SHALL be audited. *Acceptance criterion:* Installing a package with a duplicate slug or an incompatible package-format fingerprint fails with the reason named; removing a package with a Novel still bound to it returns `[ERROR] [STATE_CONFLICT]`; `ruleset (action: list)` distinguishes loaded from installed-but-idle packages. _Check:_ T453.
