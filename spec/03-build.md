...
runs use defaults from the tables below (defaults: `build` when offline, `build + manage_synthesis (action: run)` when network detected).
...
| --- | ---------------------------- | ---------------------------------------- | ------- |
| Q0  | What workflow(s) should Holonovel run? | convert / build / manage_synthesis (action: run) / update (select one or more) | build + manage_synthesis (action: run) (when network detected), build (when offline) |
...
`manage_synthesis (action: run)`; the operator may still deselect it.
...
narrative roleplay surface: scene management (`manage_scene (action: set)` with scene_type and
...
or model any ruleset content; the server's `manage_ruleset (action: search)` tool returns empty results, its
...
**Synthesis workflow.** Asked when `manage_synthesis (action: run)` is selected.
...
| 2     | Index: anchor tree, search, `manage_ruleset (action: search)` tool              | RULESET_MODEL.md anchors match source                        |
...
`manage_character (action: sheet)` tool SHALL support `markdown` (default), `json`, `html`, and
...
sub-workflows — all others (S1, S5, S6, S10–S37) — always execute, as they
...
surfaces — `manage_character (action: sheet)`, `manage_session (action: recap)`, `spec_health`, `badge_briefing`,
...
13. **Novel isolation** — entities, adventures, generated content do not leak between Novels. (Blocking.)
14. **Edge cases** — (a) moved to S1 (invalid params validated per category);
    ...
    (i) adversarial input: `manage_scene (action: set)` with SQL-injection string stores and
...
15. **Stress and recovery** — (a) two connections sharing one data directory: reads reflect
    ...
    deterministic seeds → round counter reaches 50, conditions persist, `manage_session (action: recap)`
...
17. **Novel lifecycle and persistence** — create/resume/end/switch cycle works; state persists
    to disk and restores; `manage_novel (action: end)` confirmation workflow removes file + backup; ended
    Novel blocks resume and switch. (Blocking.)
18. **Adventure generation and encounter lifecycle** — `manage_adventure (action: generate)` produces Novel-scoped,
    badge-filtered, searchable content; regeneration replaces prior; `manage_adventure (action: generate_encounter)`
...
21. **Campaign endurance** — 2 entities, 3 NPCs, 2 countdowns, 3 lore entries across 30
    ...
    chain), `manage_session (action: recap)` returns correct final state, memory hasn't doubled,
...
23. **Narrative features sweep** — exercise the narrative tool surface end to end:
    `manage_novel (action: save_context)` / `manage_novel (action: get_context)` round-trip with auto-captured faction
    clocks, countdown positions, NPC dispositions, and entity relationships;
    `manage_novel (action: end)` clears gm_context; `manage_faction (action: create)` with faction-type countdown,
    ...
    `badge_briefing`; `manage_note (action: set)` / `manage_note (action: list)` / `notes://<key>` round-trip, Player badge
    ...
    decrements on `manage_novel (action: resume)`. All mutations appear in audit log and
    `manage_session (action: recap)`. (Blocking.)
...
25. **State durability: backups, checkpoints, clones** — with
    ...
    `[restored-from-backup]` audit entry; `manage_novel (action: end)` removes all backups.
    `manage_novel (action: checkpoint_set, "a")` → 5 mutations → `manage_novel (action: checkpoint_restore, "a")` with `[NEED_INPUT]`
    confirm → assert all 5 mutations reversed; `manage_novel (action: checkpoint_remove)` removes entry;
    `TTRPG_MAX_CHECKPOINTS=1` overflow discards oldest; checkpoint survives restart
    and Novel switch; `manage_novel (action: export, json, include_checkpoints=true)` includes
    checkpoints key; Player badge returns `[FORBIDDEN]`. `manage_novel (action: clone, "src", "dst")`
...
26. **Narrative POV** — import two entities; `manage_character (action: set_active, "char_01")` — assert
    `badge_briefing` includes POV directive naming char_01 with narrative instruction
    and personality fields; `manage_character (action: set_active, "char_02")` — assert directive updates
    to char_02. `manage_character (action: set_active, "char_01", pov="omniscient")` — assert
    `badge_briefing` shows "POV: none — narration is omniscient" with char_01 still
    active; `manage_character (action: set_active, "char_02")` preserves omniscient mode;
    `manage_character (action: set_active, "char_02", pov="character")` switches to character-locked POV
...
27. **Synthesis lifecycle with Wisdom mechanical enactment** — requires
    ...
    `manage_synthesis (action: toggle, module_name, false)` — assert items from that module
    absent from synthesis surfaces. Reactivate — assert items return.
    `manage_synthesis (action: revert)` — assert community synthesis items removed, ruleset-native
    ...
    NPC — assert `manage_character (action: sheet)` renders voice_examples, goals, and personality
    patterns without manual `manage_character (action: voice)`/`manage_character (action: personality)` (P6). Create a
    countdown — assert it advances on `manage_scene (action: set)` (P7). Call
    `run_command (action: suggest, "spring a trap on the goblins")` — assert constraint override from
...
28. **Briefing ordering, voice examples, session notation** —
    `manage_session (action: briefing_order, ["scene", "entities", "lore"])` — assert `badge_briefing`
    sections in that order; unknown token returns `[INVALID_INPUT]` with valid tokens
    enumerated; `manage_session (action: briefing_order, [])` resets to builder defaults.
    `manage_character (action: voice, entity_id, [{context:"greeting", dialogue:"Hello",
    tag:"formal"}])` — assert `entity://<id>/voice_examples` returns examples;
    `entity://<id>/personality` reflects `manage_character (action: personality)` fields.
    `manage_session (action: recap, format="lonelog")` — assert output in Lonelog notation (`###` scene
    headers, `@` actions, `=>` outcomes); `manage_session (action: compress, format="lonelog")` —
    assert compressed Lonelog entries. (Non-blocking.)
29. **Novel export/import cycle** — `manage_novel (action: export, "json")` → `manage_novel (action: import, data,
    "dry-run")` reports changes without side effects → `manage_novel (action: import, data, "replace")`
    restores exported state → re-`manage_novel (action: export, "json")` matches original.
    `manage_novel (action: export, "json", "lore")` produces lore-only payload. `manage_novel (action: import, data,
...
33. **Wisdom mechanical enactment** — build with synthesis active on all 7 modules.
    Create entity and NPC while Wisdom is active — assert NPC `manage_character (action: sheet)` shows
    auto-populated voice_examples with `[vendor]` tag, goals from Wisdom patterns,
    personality without manual `manage_character (action: personality)`/`manage_character (action: voice)` calls (P6).
    Create countdown — assert `manage_countdown (action: advance)` auto-applies on `manage_scene (action: set)`, 1
    tick per transition (P7). Call `run_command (action: suggest, "negotiate with the guard")` —
...
36. **Decision chain exercise** — create vow via `manage_vow (action: set)`. Assert coupled
    countdown auto-created (P4). Call `manage_scene (action: choices, "Investigate the gate",
    [{id:"investigate_gate", label:"Check the gate"}])` with `id` matching vow
    scope — assert vow countdown advances one tick (P12). Call `manage_vow (action: milestone)`
    ...
    Call `manage_vow (action: forsake)` — assert coupled countdown removed. (Non-blocking.)
37. **Coupling advisory sweep** — create a countdown with `scope` naming a room. Set a
    secret whose text overlaps the countdown scope — call `manage_lore (action: reveal)` —
    assert a countdown-advancement advisory in `narrative_threads` (P19). Add a lore
    entry with temporal-urgency language — assert a countdown-creation suggestion in
    `narrative_threads` (P19). Set a vow whose name keyword-matches a lore trigger —
    assert `[vow-relevant]` lore surfaces (P51). Flip a relationship from `ally` to
    `rival` with a matching countdown scope — assert a countdown-advancement advisory
    in `narrative_threads` (P18). Fire the countdown with an NPC whose `location`
    matches the countdown `scope` — assert the NPC disposition shifts one step toward
    the countdown `direction` (P15). Set a secret with a `world_target` room — assert
    `[world-linked]` in `narrative_threads` (P21). Create a faction with a `territory`
    room and set the scene there — assert `[territorial]` in `narrative_threads` (P22).
    Set a server note with `narrative_tag` — assert it surfaces in `badge_briefing`;
    set a server note with temporal-urgency keywords — assert a countdown-creation
    suggestion in `narrative_threads` (P23, P49). (Non-blocking.)

**REQ-108a — Pattern Buffer traceability (Part a).**
Every Pattern Buffer sub-workflow exercises each requirement in the covered sections. The covered sections include §5.5 (Badges and Access) and §5.6 (State, Lifecycle, Entities, and Adventure Content). The sections also include §5.7 (Determinism, Safety, and Performance) and §5.8 (Synthesis, Lore, and Macros). The sections also include §5.10 (World-Model Layer), §5.12 (Narrative Architecture), and §5.13 (Holodeck). The coverage also includes the error contracts of REQ-002 (Error taxonomy). The builder records a sub-workflow-to-REQ mapping in DECISIONS.md (6) — one entry per covered REQ, naming the sub-workflow(s) that exercise it. When a REQ in these sections changes during a spec-driven update (REQ-098), the builder re-examines every sub-workflow mapped to it.

**REQ-108b — Pattern Buffer traceability (Part b).**
Gaps — a REQ in the covered sections with no mapped sub-workflow — go into the log as process-compliance findings and must resolve before handoff. New REQs added to the covered sections during a spec revision require the builder to propose at least one new Pattern Buffer sub-workflow exercising their contract; the proposal is a finding, not a blocker. _Check:_ T107.
**REQ-108c — Pattern Buffer coverage-map parity (Part c).**
The §6.6 coverage map SHALL list every REQ in the REQ-108a covered sections. A covered-section REQ with neither a map row nor an explicit non-Pattern-Buffer disposition is a validation error that blocks assembly. *Acceptance criterion:* a covered-section REQ with no map row and no disposition fails validation; a covered-section REQ with an explicit disposition row passes. _Check:_ T538.
**REQ-141a — Input-validation convergence metric (Part a).**
The convergence handshake in §6.6 must map Pattern Buffer failures to four convergence metrics. The handshake adds "input-validation gap" to the existing three (MUST-coverage gap, mechanics-fidelity defect, and process-compliance omission). A sub-workflow failure attributable to incorrect input handling — malformed parameters accepted without error, valid inputs rejected, error categories misclassified, or corrective-action text missing — maps to the input-validation metric. The builder re-enters Phase 2 of the convergence loop (§6.5) for only the affected metric.

**REQ-141b — Input-validation convergence metric (Part b).**
A failure that maps to no metric is a novel defect class and re-enters Phase 2 with all four metrics in scope. An input-validation failure goes into DECISIONS.md (6) with the failing input value, the error category returned (or absent), and the expected error category per REQ-002. The metric covers Pattern Buffer sub-workflow S14 (Edge cases) and any other sub-workflow exercising REQ-001 (Response contract) or REQ-002 (Error taxonomy) through their input contracts. _Check:_ T163.

**REQ-141c — Input-validation convergence metric (Part c).**
A single S21 execution that exceeds 10 minutes of wall-clock time passes, but the server records the actual duration. Three consecutive S21 runs exceeding the budget trigger a scope re-evaluation recorded in DECISIONS.md (5). **Per-scenario budget.** Each sub-workflow must complete within 5 minutes of wall-clock time, except S13 (10 minutes), S21 (10 minutes), S25 (10 minutes), S22 (3 minutes), and S26 (3 minutes). A sub-workflow exceeding its individual budget passes, but the server records the actual duration in DECISIONS.md (6).

**REQ-141d — Input-validation convergence metric (Part d).**
Three consecutive runs of the same sub-workflow exceeding its budget trigger a scope re-evaluation recorded in DECISIONS.md (5). **Global budget.** The full Pattern Buffer run of all sub-workflows must complete within 60 minutes of wall-clock time. The server records a run exceeding the budget with actual duration and per-sub-workflow timings in DECISIONS.md (6).
...
A Pattern Buffer failure that maps to no convergence metric under these rules goes into the log as a process-compliance finding. The builder records the novel defect class in DECISIONS.md (6) with a proposed metric mapping for future builds. _Check:_ T250.
...
Performance), §5.8 (Synthesis, Lore, and Macros), §5.10 (World-Model Layer), §5.12 (Narrative Architecture),
§5.13 (Holodeck), and REQ-002 (Error taxonomy) to at least one Pattern Buffer sub-workflow
...
| REQ-084 | S29 | run_command (action: suggest) |
| REQ-084a | S29 | manage_lore (action: suggest) |
...
| REQ-201 | I5, I10 | manage_world (action: convert) validation |
| REQ-202 | I5 | manage_world (action: convert) on populated model |
...
| REQ-165 | S16, S19 | Entity ownership personality gating |
| REQ-166 | S16, S19 | Personality briefing rendering |
| REQ-167 | S16 | Personality resource URIs |
| REQ-168 | S21 | Audit resource |
| REQ-171 | S18 | Adventure content validation |
| REQ-173 | S22 | Connection counter |
| REQ-174 | S21, S28 | Significant-roll criterion for recap |
| REQ-175 | S21 | Confrontation summary derivation |
| REQ-176 | S17 | Entity removal |
| REQ-178 | S17 | Roster listing |
| REQ-180 | S22 | Output truncation budget unit |
| REQ-211 | S22 | Evidence record field contract |
| REQ-275 | — (independent verification §10) | Evidence hash commitment |
| REQ-276 | — (independent verification §10) | Independent verifier model criteria |
| REQ-281 | S19 | Narrative-threads section token |
| REQ-286 | S19 | Knowledge-state section token |
| REQ-252 | S16, S23 | Narrative fast-forward |
| REQ-279 | S21, S28 | Narrative orientation |
| REQ-282 | S16, S19 | NPC voice directive |
| REQ-285 | S23, S24 | Server notes |
| REQ-289 | S16, S36 | Vow tracking |
| REQ-292 | S18 | Adventure catalog |
| REQ-302 | — (convergence Phase 1 metric) | Per-section content hashing |
| REQ-311 | S16, S34 | NPC memory model |
| REQ-313 | — (build process §6.7) | Server implementation fingerprinting |
| REQ-314 | — (build process §6.7) | Fingerprint-driven partial rebuild |
| REQ-213 | S7 | Weighted table result |
| REQ-251 | S18 | Generation intent guard |
| REQ-253 | S24 | Tool-output verbosity control |
| REQ-273 | — (independent verification §10) | Reproducibility tolerance |
| REQ-274 | — (independent verification §10) | Verifier confidence score |
| REQ-291 | S16 | Oracle tool |
| REQ-312 | S16 | Pre-narration validation |
| REQ-433 | S24 | Event notification surface |
| REQ-185 | S19 | Section token vocabulary |
| REQ-186 | S19 | Section token discoverability |
| REQ-226 | — (convergence Phase 1 metric) | Narrative voice profile extraction |
| REQ-227 | S27 | Two-tier synthesis model |
| REQ-228 | S27 | Synthesis consistency during updates |
| REQ-230 | S27 | Synthesis status dashboard |
| REQ-231 | S27 | Per-module synthesis toggle |
| REQ-234 | S23, S34 | Secrets and knowledge |
| REQ-243 | S27 | Synthesis population during updates |
| REQ-244 | — (convergence Phase 1 metric) | Convergence cache key |
| REQ-245 | — (convergence Phase 1 metric) | Pre-computed synthesis manifest |
| REQ-246 | S16, S23 | Story journal |
| REQ-260 | S27 | Granular synthesis activation |
| REQ-261 | S27 | Player synthesis |
| REQ-262 | S27 | Novel synthesis tool |
| REQ-263 | S27 | Novel synthesis run |
| REQ-264 | S27 | Novel synthesis auto-trigger |
| REQ-265 | S27 | Synthesis in badge_briefing |
| REQ-266 | S27 | Novel synthesis confidence |
| REQ-310 | S16, S21 | Campaign memory |
| REQ-328 | S16, S32 | Lore-world coupling |
| REQ-222 | I17 | Parser vocabulary extension |
| REQ-431 | I5 | Procedural world generation |
| REQ-335 | S16, S35 | Scene beat taxonomy |
| REQ-336 | S35 | Dramatic pacing signal |
| REQ-337 | S16 | Narrative arc visibility |
| REQ-338 | S23, S35 | Faction autonomous advancement |
| REQ-339 | S35 | NPC goal pursuit |
| REQ-340 | S35 | Discovered consequences |
| REQ-341 | S16, S19 | Player-facing spatial surface |
| REQ-342 | S16, S32 | Scene description from world model |
| REQ-343 | S16, S32 | Unified intent resolution |
| REQ-344 | S16, S28 | Voice example feedback |
| REQ-345 | S16, S34 | Background-derived knowledge |
| REQ-346 | — (G7 narrative-coherence attestation) | Narrative coherence attestation |
| REQ-347 | S16, S28 | Voice feedback codex capture |
| REQ-348 | S35 | Faction-NPC goal coordination |
| REQ-349 | S35 | Consequence-to-knowledge coupling |
| REQ-350 | S16, S32 | Background lore triggering |
| REQ-351 | S35 | Pacing-triggered autonomy |
| REQ-352 | S16, S18 | Codex adventure beat sequences |
| REQ-353 | S16, S35 | Beat-accelerated countdown advancement |
| REQ-355 | S37 | Secret-countdown coupling |
| REQ-356 | S37 | Vow-lore coupling |
| REQ-357 | S32, S36 | Story journal-faction coupling |
| REQ-358 | S37 | Countdown-NPC disposition coupling |
| REQ-359 | S37 | Relationship-countdown coupling |
| REQ-360 | S37 | Lore-countdown coupling |
| REQ-361 | S36 | NPC-vow coupling |
| REQ-362 | S36 | Faction-vow coupling |
| REQ-363 | S37 | Secret-world coupling |
| REQ-364 | S37 | Faction-world coupling |
| REQ-365 | S37 | Server notes narrative coupling |
| REQ-366 | S19 | Observer narrative surface |
| REQ-376 | — (Holonovel Pattern Buffer traceability) | Holonovel sub-workflow-to-REQ mapping |
...
The full 37-sub-workflow Pattern Buffer is not required when the
...
selection. The full 37-sub-workflow Pattern Buffer SHALL still execute when the
...
intake, forcing all 37 sub-workflows regardless of fingerprint match.
...
4. **CRUD round-trip** — create a room via `manage_world (action: create_room)`, create a thing in it,
...
5. **manage_world (action: convert) with fixture** — call `manage_world (action: convert)` with the Appendix K
   ...
   content. Call `manage_world (action: convert)` on the same Novel — assert `[STATE_CONFLICT]`.
   (Blocking.)
...
6. **Property state propagation** — open a closed container, assert contents
   ...
   appear in audit log and `manage_session (action: recap)`. (Blocking.)
...
8. **Large-map navigation** — populate 50+ room world model. Navigate from one
   ...
   correct, no state corruption, memory stable. `manage_session (action: recap)` covers
   traversal history.
...
10. **Hybrid adventure load** — load an adventure module containing `## World`
    assertions (Appendix K fixture format) via `manage_adventure (action: load)`. Assert
    world-model tier populated, room descriptions match, things placed in
    declared rooms, exits connected. Assert `manage_ruleset (action: search)` finds adventure
    prose. Assert `badge_briefing` surfaces adventure content badge-filtered.
    (Blocking.)

11. **Narrative CRUD cycle** — create an NPC via `manage_npc (action: create)`, set personality
    fields via `manage_character (action: personality)`, attach voice examples via `manage_character (action: voice)`,
    update the NPC's disposition via `manage_npc (action: update)`, then remove it via
    `manage_npc (action: remove)`. Assert `badge_briefing` surfaces NPC name and disposition after
    each mutation. Assert `manage_session (action: recap)` covers the create-update-remove
    sequence. Assert `manage_npc (action: remove)` on a nonexistent NPC returns `[NOT_FOUND]`.
    (Non-blocking.)

12. **Lore and countdown lifecycle** — create a lore entry with triggers via
    `manage_lore (action: set)`, toggle it disabled then re-enabled via `manage_lore (action: toggle)`,
    update its content via `manage_lore (action: update)`, remove it via
    `manage_lore (action: remove)`. Create a countdown via `manage_countdown (action: set, ticks=2)`,
    advance it twice to expiry via `manage_countdown (action: advance)` — assert the expiry audit
    log entry and countdown removal. Assert `manage_lore (action: remove)` on a removed
    entry returns `[NOT_FOUND]`. (Non-blocking.)

13. **Scene state and guidance** — set scene state via
    `manage_scene (action: set, description, location, time_of_day, atmosphere)`, set scene
    ...
    custom briefing order via `manage_session (action: briefing_order, [...])` — assert `badge_briefing`
    sections appear in the specified order. Assert `manage_scene (action: set)` transitions
    push the prior scene to `scene_history`. (Non-blocking.)

14. **Device lifecycle** — create a device via `manage_world (action: create_thing, "lantern", {kind:
    ...
    Assert `manage_world (action: convert)` recognizes "It is switchable." and "It is switched
    on." (Blocking.)

15. **Vehicle lifecycle** — create a world model with a vehicle via
    `manage_world (action: convert)`. Assert `command("enter raft")` returns `[OK]` and
...
16. **Extended property contracts** — create things with `wearable`, `edible`,
    ...
    `manage_world (action: convert)`. Assert each property assertion is recognized. Assert
...
| manage_world (action: convert), hybrid parsing                     | 5, 10                     |
...
initially `pending`, updated to `converged` when the builder resolves the discrepancy.
*Acceptance criterion:* When G2 fails on an manage_combat (action: init) turn-order mismatch,
...
**REQ-098 — Spec-driven update workflow.** When the operator updates an existing MCP server
...
| Major    | State model changed, new tools/prompts/resources mandated, badge-gating contract altered | Full gap audit; full Pattern Buffer (§6.6 — 37 sub-workflows, scoped per the §6.6 surface-to-scenario and fingerprint rules) |
