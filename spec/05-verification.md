...
manage_session (action: recap) correctness (REQ-072), and undo round-trip (REQ-041). Wording is
...
be asserted exactly. Natural-language prose in `manage_scene (action: set)`,
`manage_session (action: recap)`, narrative tool output, and error corrective-action text SHALL
...
absent from `badge_briefing` and `run_command (action: suggest)` output; `synthesis://status`
reports correct per-module active/inactive counts; `manage_synthesis (action: revert)` removes all
...
| ----------------------------- | ------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Power Gamer                   | Stacks non-stacking bonuses                                                    | `[ERROR] [RULE_VIOLATION]`, or `[PARTIAL]` with explanation                                                                             | As Player, calls `manage_condition (action: apply)` with a condition already active on the target entity.                                          |
...
| Curious Player                | Invokes a GM-only tool                                                    | `[ERROR] [FORBIDDEN]` stating the restriction                                                                                           | As Player badge, calls `manage_combat (action: init)`.                                                                                          |
| Rules Lawyer                  | Cites ambiguous wording to demand an outcome                                   | `[PARTIAL]` explaining the conflict and citing both texts, or `[OK]` returning the raw rule text                                        | Calls `manage_ruleset (action: search)` on a topic the ruleset defines in two conflicting sections.                                                  |
...
2. **Search isolation.** `manage_ruleset (action: search, "core mechanic")` under a ruleset A
...
4. **Import rejection.** Export a ruleset-A Novel. Build a new host
   that does not include ruleset A. Attempt to `manage_novel (action: import)` — assert
...
5. **Cross-ruleset Novel switching.** Create Novel A (ruleset A). Create
   ...
   `manage_novel (action: switch)` to Novel B. Call the same ruleset-A tool — assert per
...
8. **Codex isolation.** Call `manage_codex (action: capture, "npc", name)` from a ruleset-A
   Novel — assert the created entry carries `ruleset` set to ruleset A's slug.
   Call `manage_codex (action: import)` of that entry into a ruleset-B Novel — assert
   `[ERROR] [STATE_CONFLICT]` naming both rulesets.

9. **Load-time namespacing audit.** After each `manage_ruleset (action: install)`, assert every
   ...
   `manage_ruleset (action: remove)` deregisters all of the package's tools and prompts while no
