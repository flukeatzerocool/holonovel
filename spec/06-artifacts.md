...
The Novel export format (§7.7, `novel (action: export)`) is the runtime equivalent of a holo-novel
...
| H18   | T443 | Per-ruleset search and lookup isolation      | `ruleset (action: search)` and canonical lookups under each Novel return only that ruleset's results. |
