...
The Novel export format (§7.7, `manage_novel (action: export)`) is the runtime equivalent of a holo-novel
...
| ----- | -------- | ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| H1    | T36      | Compare DECISIONS.md (1) intake answers against package set       | For multi-ruleset builds, compare the slug-to-path mapping against the host's `ruleset_prefix_map` and each `<slug>_RULESET_MODEL.md`. Single-ruleset behavior unchanged. Per Standing Rule 9, ruleset-free builds pass with the recorded "ruleset-free — no source" entry in DECISIONS.md (1). |
...
| H7    | T41      | Instrument server, run a canonical lookup              | No tool handler reads ruleset Markdown files after startup indexing or package load; canonical lookups use the loaded package's index or model. |
| H8    | T43      | Start a workflow, verify no auto-completion            | A workflow that raises `[NEED_INPUT]` does not complete without a `respond_decision` call; no option is pre-selected.           |
...
| H16   | T441 | Create Novel per ruleset, verify binding     | Each Novel's `ruleset` field matches the creation parameter; immutable except the audited migration path (REQ-380c); `manage_novel (action: create)` rejects unknown slugs. |
| H17   | T442 | Cross-ruleset tool call rejection            | A ruleset-A tool called against a ruleset-B Novel returns `[ERROR] [INVALID_INPUT]` naming the active ruleset. Infrastructure tools succeed under any Novel. |
| H18   | T443 | Per-ruleset search and lookup isolation      | `manage_ruleset (action: search)` and canonical lookups under each Novel return only that ruleset's results. |
