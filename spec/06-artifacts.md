...
| H16   | T441 | Create Novel per ruleset, verify binding     | Each Novel's `ruleset` field matches the creation parameter; immutable except the audited migration path (REQ-380c); `create_novel` rejects unknown slugs. |
...
| H18   | T443 | Per-ruleset search and lookup isolation      | `search_rules` and canonical lookups under each Novel return only that ruleset's results. |
