...
| T31   | Automated | Novel isolation: entities invisible across Novels; roster baselines immutable; `character (action: import)` creates fresh copy; `novel (action: end)` discards Novel; roster survives; resuming ended Novel fails                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | REQ-055                                     |
...
| T471 | Automated | Session no-mutation: open a session (new `TTRPG_SESSION_ID`), make zero mutating calls, close the session — assert `spec_health` and `session (action: recap)` surface a `[session-no-mutations]` marker naming the session; assert no tool is blocked by the marker. | REQ-402, REQ-237 |
| T472 | Automated | State-drift gate: set `gm_context` via `novel (action: save_context)` with no intervening state mutation — assert `[state-drift]` appears in `spec_health`, `session (action: recap)`, and the `state_ledger` token. With `TTRPG_STATE_GATE=warn`, assert session-close surfaces append a warning naming the uncommitted beats. With `TTRPG_STATE_GATE=block`, assert `novel (action: save_context)`, `novel (action: end)`, and `novel (action: switch)` return `[STATE_CONFLICT]` while drift is active, and that a commit call clears the gate. With `TTRPG_STATE_GATE=off`, assert no blocking. | REQ-403 |
| T473 | Automated | Roll-to-commit: perform a significant roll without a following state mutation — assert `session (action: recap)` flags `[uncommitted-roll]` naming the roll and suggested commit tool. Perform a significant roll followed by a state mutation — assert no flag. Perform a `forged (action: action_roll)` without a following commit — assert `[uncommitted-roll]` names `forged (action: stress)` as the suggested commit tool. | REQ-404, REQ-174 |
| T474 | Automated | Auto-moment: with `auto_record` defaulting true, call `scene (action: set, "The vault")` — assert a `moment` story journal entry is appended with scene anchor and timestamp. Call with `skip_transition_hook=true` — assert no auto-moment. Set the Novel `auto_record` false — assert `scene (action: set)` appends no moment. | REQ-405, REQ-246, REQ-125 |
...
| T481 | Automated | Oracle player access: call `scene (action: oracle, "Is there a guard behind the door?", "50_50", seed="42")` under the Player badge — assert returns `[YES]`, `[NO]`, `[EXCEPTIONAL_YES]`, or `[EXCEPTIONAL_NO]`. Call under the Game Master badge — assert succeeds. Assert no badge returns `[FORBIDDEN]` for `scene (action: oracle)`. | REQ-291 |
...
| T55   | Automated | Multi-entity: create two entities, import both into a Novel, assert `entities://` lists both. Switch active entity via `character (action: set_active)`, assert mutating tools target the active entity. Verify `party://current` lists all player entities with summary stats.                                                                                                                                                                                                                                                                                                                                                                                                                                       | REQ-074                                     |
...
| T57   | Automated | Scene state: set scene state, verify it appears in `scene://current` and `badge_briefing`. Update scene state, verify old entry in audit log and new entry as current. Attempt `scene (action: set)` as Player badge — assert `[FORBIDDEN]`.                                                                                                                                                                                                                                                                                                                                                                                                                                                         | REQ-076, REQ-032                            |
...
| T59   | Automated | Adventure load: load an adventure, verify `adventure://<slug>/<anchor>` resources are retrievable. Assert `*Keeper only*` sections return content for Game Master badge and are hidden from Player badge. Assert `ruleset (action: search)` with a query matching a section heading assigns HIGH confidence; matching body text assigns MEDIUM confidence. Assert each result line begins with a bracketed confidence label matching the query-token location rule. Assert `[generated]` tag does not affect sort order.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | REQ-079, REQ-032                            |
| T60   | Automated | Adventure isolation: load adventure A, create NPCs from its text. Load adventure B via `adventure (action: load)`. Assert adventure A's NPCs persist as Novel entities but adventure A's content no longer appears in `badge_briefing`. Verify no content leak between adventures.                                                                                                                                                                                                                                                                                                                                                                                                                              | REQ-079                                     |
...
| T63   | Automated | Synthesis boundaries: run synthesis (action: run), diff entity stat fields (stats/saves/HP) before and after — assert no changes. Diff `tools/list` — assert no changes. Assert all voice_examples, lore templates, and action patterns carry `[supplementary]` tag. Assert all seven synthesis output modules (§11.1) are populated with non-empty content. Re-run synthesis (action: run) — assert idempotent. Switch to player badge — assert GM-scoped synthesis content hidden.                                                                                                                                                                                                                                                                                                                     | REQ-080, REQ-077, REQ-032                   |
...
| T70   | Automated | Audit compression: run several mutations (advance combat, apply condition). Call `session (action: compress, 3)` — assert output contains Markdown header "Compressed audit log (summarize into a single paragraph):" followed by per-entry lines in format `[timestamp] [badge] tool_name — output_prefix`. Assert forbidden-call entries carry `[BOUNDARY_VIOLATION]` prefix. Switch to Player badge — assert entries affecting a player-owned entity are visible even when the recorded badge is `game_master`. Verify audit log is unchanged (REQ-040). Call with 0 — assert `[ERROR] [INVALID_INPUT]`.                                                                                                                                                                                                                                                                                                                                                                                          | REQ-086, REQ-032, REQ-040                   |
| T71   | Automated | Scene type tagging: set scene type to "social" — assert GM `badge_briefing` prioritizes social tools in registry section. Call `command (action: suggest, "talk")` — assert social actions appear. Call `combat (action: init)` — assert combat tools prioritized in `badge_briefing` regardless of scene type. Set to unknown type — assert `[ERROR] [NOT_FOUND]` with valid values enumerated. Player attempt returns `[FORBIDDEN]`. Restart — verify type persists.                                                                                                                                                                                                                                                                                                     | REQ-087, REQ-032                            |
| T72   | Automated | Novel lifecycle: create Novel, assert state file on disk at `.holonovel-state/novels/<slug>.json`. Restart server with same `TTRPG_NOVEL`, assert state restored (entities, NPCs, scene). `novel (action: end)`, assert file removed from disk. Resume ended Novel → `[STATE_CONFLICT]`. Create Novel with duplicate slug → `[STATE_CONFLICT]`. Server start without `TTRPG_NOVEL` — Novel-scoped tools return `[STATE_CONFLICT]`. This test reads the on-disk state format — it verifies REQ-092's format contract (verification workflow G4). Pattern Buffer sub-workflows (G5) verify the same state-survival behaviors through tool-observable surfaces. See §6.6 Verification principle.                                                                                                                                                                                                                                                                                   | REQ-088, REQ-092                            |
...
| T75   | Automated | Adventure generation: call `adventure (action: generate, "A haunted space station")`. Assert output contains title, Overview (GM-only), Adventure Hook, 2–6 locations, NPC entries. Assert generated content retrievable at `adventure://<slug>/<anchor>`. Assert GM-only sections hidden from Player. Assert appears in `ruleset (action: search)` results. Regenerate — assert old content replaced.                                                                                                                                                                                                                                                                                                                       | REQ-090, REQ-032                            |
| T76   | Automated | Enhanced encounters: call `adventure (action: generate_encounter, "dark alley")`. Assert output creates a scene_state entry, an NPC, and a lore entry — all snapshot-able. Call without context — assert generates from ruleset tables. Undo — assert all three artifacts removed (single undo). Player attempt → `[FORBIDDEN]`.                                                                                                                                                                                                                                                                                                                                                                                           | REQ-091, REQ-041, REQ-032                   |
...
| T82   | Automated | Lore suggestion: run synthesis (action: run) (or seed mock templates), call `lore (action: suggest)` with and without scene text — assert up to 5 matching templates returned with key, content preview, triggers, confidence, and source_url. Call `lore (action: suggest)` with no synthesis run — assert empty list with synthesis guidance note. Verify no template fabrication. Switch to Player — assert GM-scoped templates excluded. Switch to GM — assert `lore (action: suggest)` returns templates of all `badge_scope` values (game_master, shared, and any player-scoped templates).                                                                                                                                                                                                                                                                                                                                        | REQ-083, REQ-032, REQ-080                   |
...
| T94   | Automated | Synthesis reversion: run synthesis (action: run), verify 7 modules populated. Call `synthesis (action: revert)` — assert all synthesis resource URIs (`synthesis://voice_examples`, `synthesis://briefing_order`, `synthesis://action_patterns`, `synthesis://adventure_advice`) return empty or absent; `lore://templates` returns only Novel-scoped entries, synthesis state removed, mechanical fields unchanged, `[ruleset]` content unchanged, DECISIONS.md synthesis evidence retained, GM-configured briefing_order and action_patterns_enabled survive reversion unchanged. Re-run synthesis (action: run) — assert repopulation succeeds. Player badge attempt returns `[FORBIDDEN]`. Assert synthesis briefing_order tokens are a subset of `spec_health.section_tokens`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | REQ-103, REQ-080, REQ-185                            |
| T95   | Automated | LOW-confidence tagging: run synthesis (action: run) with LOW items present. Inspect `badge_briefing` and synthesis resources — assert every LOW-confidence item carries `[LOW]` tag distinct from `[supplementary]`. Assert LOW items appear after HIGH/MEDIUM items within their module section. Assert HIGH/MEDIUM items do not carry `[LOW]` tag.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | REQ-080                                     |
| T96   | Automated | Action pattern inertness: run synthesis (action: run). Assert `command (action: suggest, intent)` does not return synthesis-derived patterns while the action pattern toggle (REQ-115) is disabled. Activate patterns via `synthesis (action: toggle_action)` — assert patterns appear in results for matching intents. Deactivate via `synthesis (action: toggle_action)` — assert patterns excluded again. GM-only tool patterns excluded from Player results whether activated or not. Player badge attempt on `synthesis (action: toggle_action)` returns `[ERROR] [FORBIDDEN]`.                                                                                                                                                                                                                                                                                                                                                                                                                                                      | REQ-084                                     |
| T97   | Automated | Synthesis collected_at: run synthesis (action: run). Inspect every item in all seven output modules — assert `collected_at` is present, non-empty, valid ISO 8601, and within ±1 minute of current time.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | REQ-080                                     |
| T98   | Automated | Novel switching: create two Novels (A and B) with distinct state. Switch from A to B via `novel (action: switch)` — assert B's state restored independently. Switch back to A — assert A's state unchanged. Assert `novel (action: switch)` with non-existent slug returns `[STATE_CONFLICT]`. Assert switching to ended Novel returns `[STATE_CONFLICT]`. Assert two connections with different active Novels operate independently. Verify badge state restores per Novel on switch.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | REQ-095, REQ-088, REQ-055                   |
...
| T119  | Automated | Action pattern toggle: novel has active synthesis patterns. Call `command (action: suggest)` with a pattern-matching intent — assert patterns absent. Call `synthesis (action: toggle_action)` — assert "enabled" in response. Call `command (action: suggest)` with the same intent — assert patterns appear. Call `synthesis (action: toggle_action)` — assert "disabled." Call `command (action: suggest)` — assert patterns absent again. Player badge attempt on `synthesis (action: toggle_action)` returns `[FORBIDDEN]`. No novel active — assert `[STATE_CONFLICT]`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | REQ-115, REQ-084, REQ-032                   |
| T120  | Automated | Suggestion precision: call `command (action: suggest, "I want to convince the guard")` — assert at least one result maps to a social-resolution tool, not a combat or lookup tool. Call `command (action: suggest, "strike a bargain")` — assert results exclude weapon-attack tools. Call with a combat intent while combat is active — assert combat tools appear and scene-type filtering excludes non-combat tools from the top results. Call with intent matching no plausible tool ("I want to become a sandwich") — assert empty list.                                                                                                                                                                                                                                                                                                                                                                                                   | REQ-084                                     |
...
| T126  | Automated | NPC stat block reference: call `lookup_monster("goblin")`, capture its stats. Call `npc (action: create, "Goblin Scout", reference="goblin")` — assert NPC created with stats matching the goblin entry. Call `npc (action: create, "Goblin Chief", reference="goblin", hp=21)` — assert HP overridden to 21, other stats from reference. Call `npc (action: create, "Fake", reference="nonexistent")` — assert `[ERROR] [NOT_FOUND]` with valid reference names enumerated. Call `npc (action: create)` with a reference entry carrying a field not in the builder-determined stat surface — assert the extra field is included in the NPC's stat block and resource URI output. Assert reference parameter is optional — calling without reference succeeds.                                                                                                                                                                                                     | REQ-119                                     |
| T127  | Automated | NPC rendering: create NPC with stat fields and narrative fields (per REQ-075, REQ-122). Call `character (action: sheet, npc_id)` — assert output contains NPC name, populated stat fields, conditions, and narrative fields in ruleset baseline format. Call with a non-existent ID — assert `[ERROR] [NOT_FOUND]`. Switch to Player badge — assert stat fields visible, GM-only narrative fields hidden. Verify output format matches entity `character (action: sheet)` format.                                                                                                                                                                                                                                                  | REQ-120, REQ-032                            |
| T128  | Automated | NPC resource URIs: create NPC, assert `npc://<id>` resource returns full stat block and narrative fields. Assert `npcs://` resource lists all active NPCs with name, disposition, location. Assert resources re-registered on `novel (action: switch)`. Assert resources removed after `novel (action: end)`. Switch to Player badge — assert `npc://<id>` returns summary fields only, `npcs://` returns summary list.                                                                                                                                                                                                                                                      | REQ-121, REQ-032                            |
| T129  | Automated | NPC narrative fields: create NPC. Call `character (action: personality, npc_id, description, voice, background, goals)` — assert fields set and surfaced in `badge_briefing` and at `npc://<id>/personality`. Call `character (action: voice, npc_id, [...])` — assert examples set. Verify NPC narrative fields are Novel-scoped — `novel (action: end)` removes them, no roster backing. Assert Player badge attempt on `character (action: personality)` for NPC returns `[FORBIDDEN]`.                                                                                                                                                                                                                                                                                                                            | REQ-122, REQ-075, REQ-032                   |
| T191  | Automated | NPC description field: call `npc (action: create, "Guard", description="Tall")` then `character (action: personality, npc_id, {description: "Suspicious"})` — assert the NPC's description reads "Suspicious" at `npc://<id>`, `character (action: sheet)`, and `npc://<id>/personality`. Call `npc (action: create, "Merchant")` (no description) then `character (action: personality, npc_id, {description: "Cheerful"})` — assert description is "Cheerful" at all three surfaces.                                                                                                                                                                                                                                                                                                                 | REQ-156                                     |
| T192  | Automated | Combat determinism: start a fresh server with `TTRPG_SEED=7`. Call `combat (action: init)` with one danger and `seed="42"` — assert danger initiative d20 face matches Appendix B.4 seed-42 column at position 1 (value 6). Call `roll_save("dexterity")` without a seed — assert the d20 face matches the session sequence (seed-7 B.4 column). Call `combat (action: init)` with two dangers and `seed="42"` — assert d20 faces match positions 1 and 2 of the B.4 seed-42 column. Call `combat (action: init)` with one danger and no seed — assert the d20 face matches the next position in the seed-7 session sequence (after the roll_save draw). | REQ-157                                     |
...
| T194  | Automated | Synthesis briefing integration: after synthesizing a Novel, invoke `badge_briefing` as GM — assert supplementary guidance items appear tagged `[supplementary]` with source URLs. Assert synthesis-sourced voice examples appear under entity personality with `[supplementary]` tag. Switch to Player badge — assert game_master-scoped synthesis items are absent. Call `synthesis (action: revert)` — assert all synthesis content absent from all badge briefing views.                                                                                                                       | REQ-159, REQ-080                            |
| T195  | Automated | Synthesis health reporting: after synthesizing a Novel, invoke `session (action: health)` — assert `synthesis_active: true`, per-module counts matching the manifest, non-empty fingerprint. Call `synthesis (action: revert)` — assert `synthesis_active: false` and `module_counts` contains all seven module names (`voice_examples`, `briefing_order`, `lore_templates`, `action_patterns`, `supplementary_guidance`, `adventure_advice`, `narrative_voices`) each with value zero. Populate stale items past `TTRPG_SYNTHESIS_STALE_DAYS` — assert `stale_count` increments and `[stale]` flag. Activate a lore template via `lore (action: set)` — assert `activated_count` increments.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | REQ-160, REQ-025                            |
...
| T200  | Automated | Entity ownership for personality gating: create an entity as Player via `character (action: create)` — assert `character (action: personality)` succeeds on that entity. Create an entity as GM — assert the Player can still call `character (action: personality)` on it (ownership non-exclusive). A Player who has never created any entity can call `character (action: personality)` on an entity imported by the GM.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | REQ-165, REQ-077                            |
| T201  | Automated | Personality briefing rendering: set `voice: "gruff"` and `goals: "find the relic"` on an entity via `character (action: personality)`. Assert `badge_briefing` renders both fields under the entity's name and `description`/`background` are absent. Create an entity with no personality — assert absent from the personality group. Create an NPC with narrative fields — assert NPC personality renders alongside entity personality with an NPC marker. Assert the group renders the empty-state marker when no entities have personality data.                                                                                                                                                                                                                                                                                                                                                                                                                                                            | REQ-166, REQ-109                            |
...
| T212  | Automated | Session recap entity status: create entities at full HP, at 0 HP, and with a death condition applied. Assert `session (action: recap)` reports entity status as "alive" when HP > 0, "unconscious" at HP = 0, "dead" when the ruleset death condition is active. For a ruleset without a death condition, assert statuses are "alive" and "incapacitated."                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | REQ-072                                     |
| T213  | Automated | Significant-roll criterion: run 2 saving throws, 3 weapon attacks, 2 table rolls, 1 untargeted skill check, and 1 `fate (action: roll)` with a difficulty. Assert `session (action: recap)` includes 7 significant rolls (2 saves + 3 attacks + 1 skill check + 1 fate roll) and excludes 2 table rolls. Assert roll ordering is chronological. Assert each entry has tool name, entity_id, die faces, and outcome summary.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | REQ-174, REQ-072                            |
| T214  | Automated | Confrontation summary: run combat (action: init) → combat (action: advance) (2 rounds) → combat (action: end, outcome="goblins routed"). Call `session (action: recap)` — assert `confrontations_completed` has one entry with participant count, round count = 2, outcome = "goblins routed", and `confrontation_pending` is null. Run a second combat (action: init) (no combat (action: end)) — assert `confrontations_completed` still has one entry and `confrontation_pending` is non-null with participants, round, and turn_position.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | REQ-175, REQ-072                            |
| T215  | Automated | Session recap structured output: invoke `session (action: recap)` on a populated Novel. Assert all 14 named fields are present (timespan_start, timespan_end, entities, confrontations_completed, confrontation_pending, scene, scene_type, lore_entries, narrative_directive, scene_transitions, roster_changes, condition_changes, significant_rolls, total_combat_rounds). Assert `timespan_start` and `timespan_end` are ISO 8601 strings or null. Assert `entities` array entries have `name`, `hp`, `max_hp`, `conditions`, and `status` fields. Assert `total_combat_rounds` is an integer. Assert no unlabeled narrative prose surrounds the structured fields.                                                                                                                                                                                                                                                                                                                                                                                        | REQ-072                                     |
| T216  | Automated | Entity removal: import two entities into a Novel — assert `character (action: remove, "character_02")` removes the entity from `entities://` and `party://current`. Assert the roster baseline is unchanged and re-importing the same roster ID creates a fresh copy. Assert Player badge returns `[FORBIDDEN]`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | REQ-176, REQ-074                            |
| T217  | Automated | Roster entity removal: create a roster character, import it into a Novel. Call `character (action: roster_remove, roster_id)` — assert entry removed from `roster://`. Assert the Novel entity copy survives independently. Call with nonexistent roster ID — assert `[NOT_FOUND]` with valid IDs enumerated. Assert Player badge returns `[FORBIDDEN]`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | REQ-177                                     |
| T218  | Automated | Entity cardinality: set `TTRPG_MAX_ENTITIES=1`, import two entities — assert second returns `[STATE_CONFLICT]` with group name and counts. Set `TTRPG_MAX_ENTITIES=0` — assert `character (action: import)` fails with `[STATE_CONFLICT]`. Set `TTRPG_MAX_ROSTER_ENTITIES=1`, create two characters — assert second returns `[STATE_CONFLICT]` before state mutation. Assert `spec_health` reports entity count, maximum, overflow flag, and 80% warning.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | REQ-129, REQ-074                            |
| T219  | Automated | Roster listing: create three roster characters. Assert `character (action: roster_list)` returns all with ID, name, race, class, level. Assert `roster://character_01` returns full data including personality fields. Clear roster — assert returns empty-state marker.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | REQ-178, REQ-089                            |
| T220  | Automated | Duplicate import guard: import roster character "Thorn" into Novel A. Call `character (action: import)` for the same roster ID into Novel A again — assert `[STATE_CONFLICT]` naming existing entity. Import "Thorn" into Novel B — assert success.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | REQ-074                                     |
| T206  | Automated | Load adventure error contract: call `adventure (action: load, "nonexistent-slug")` — assert `[ERROR] [NOT_FOUND]` with available adventure slugs enumerated in the error value. Assert the builder records the validation mechanism in DECISIONS.md. Call `adventure (action: load, "tomb-of-horrors")` with a valid slug — assert the adventure activates normally.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | REQ-079                                     |
...
| T130  | Automated | Builder-defined NPC stat fields: build server for a ruleset with stat-block conventions. Assert `npc (action: create)` exposes stat fields matching that ruleset's stat-block schema (not hardcoded ac/hp/speed). Build with a ruleset that has no NPC stat conventions — assert NPC surface exposes only narrative fields. Assert all stat fields are optional. Assert `name` is the only required field.                                                                                                                                                                                                                                                                                              | REQ-123, REQ-075                            |
| T131  | Automated | NPC damage resolution: create NPC with ac and hp. Initiate combat with NPC as participant. Call `combat (action: advance)` through NPC's turn — assert turn resolves. Call damage-resolution tool with NPC as target — assert damage resolved against ruleset's damage model (HP, wounds, or loss-of-effectiveness metric), result transparent per REQ-003. Reduce NPC to zero health threshold — assert incapacitation condition applied per ruleset convention. Assert damage against NPC is audited and snapshot-able. Call damage-resolution tool with unknown NPC ID — assert `[ERROR] [NOT_FOUND]`.                                                                                                                                                                                                     | REQ-124, REQ-043, REQ-003                   |
| T132  | Automated | Scene history cap: call `scene (action: set)` N+1 times (N = configured max). Assert `scene://history` returns exactly N entries (most recent). Assert output includes count of suppressed entries and `[truncated]` marker. Assert audit log contains all N+1 entries.                                                                                                                                                                                                                                                                                                                                                                                                                                                  | REQ-076                                     |
| T133  | Automated | Structured scene fields: set scene state with description, location, time_of_day, atmosphere. Assert all fields appear in `scene://current` and `badge_briefing`. Set scene state with only description — assert optional fields empty or absent. Attempt `scene (action: set)` with structured fields as Player badge — assert `[FORBIDDEN]`. Restart — verify fields persist.                                                                                                                                                                                                                                                                           | REQ-076a, REQ-032                            |
...
| T146  | Automated | Adventure generation lifecycle: call `adventure (action: generate, "A haunted space station")` — assert content at `adventure://generated/overview` and `adventure://generated/hook`. Restart server — assert generated adventure preserved. Call `adventure (action: load, "tomb-of-horrors")` — assert indexed and generated adventures coexist in `badge_briefing`, indexed first then generated. Call `adventure (action: generate, "Sunken temple")` — assert old generated content replaced. `novel (action: end)` — assert generated adventure discarded with the Novel, not present on disk in `TTRPG_ADVENTURE` directory. | REQ-132, REQ-079, REQ-090                |
...
| T148  | Automated | Minimum Player tool surface: set Player badge, invoke one tool from each Player-guaranteed group (dice-resolution, ruleset lookup, character (action: sheet), command (action: suggest), character (action: signal), help, undo, set_badge) — assert all succeed. Invoke a GM-exclusive tool (npc (action: create), combat (action: init), scene (action: set), lore (action: set)) — assert each returns `[FORBIDDEN]`. Switch to Game Master — assert all tools succeed. Verify `tools/list` filtered by each badge matches the DECISIONS.md classification table. | REQ-134, REQ-032 |
...
| T152  | Automated | Prompt health reporting: invoke `session (action: health)` — assert every registered prompt appears in a `prompt_health` section with name, presence, length, budget, budget-compliance flag, and stale-references list. Rename a tool referenced in a prompt — assert the stale-references list for that prompt shows the old tool name. Restore the original name — assert the stale-references list clears. | REQ-138 |
| T153  | Automated | Resource URI completeness: invoke `session (action: health)` — assert a `resource_uris` section lists every REQ-022 URI template with presence (present/absent), registration name, and MIME type. Register a new resource — assert its URI appears as `present` immediately. Remove a resource — assert the URI changes to `absent`. | REQ-139 |
| T154  | Automated | Gap audit comparison surface: invoke `session (action: health)` — assert a `gap_audit` section is present containing a delta summary (spec_version comparison), tool-catalog comparison (per-category presence), resource-map comparison, prompt-list comparison, and badge-gating summary. Assert the section is absent when build is not complete. | REQ-025 |
...
| T158  | Automated | End-Novel confirmation dispatch: `novel (action: end)` → assert `[NEED_INPUT]` with yes/cancel options. `respond("End Novel <slug>?", "yes")` → assert Novel file removed, active set empty. `novel (action: resume, slug)` → assert `[STATE_CONFLICT]`. `respond` with non-matching decision → assert `[NOT_FOUND]` with open decision text. | REQ-140 |
...
| T160  | Automated | Novel file-size accuracy: invoke `session (action: health)` — assert on-disk file size metric matches OS-reported size within 1%. Mismatch > 1% — assert `[size-mismatch]` warning in `spec_health`. Assert growth trajectory uses on-disk size. | REQ-097 |
| T161  | Automated | combat (action: advance) audit-log-derived output: init combat with entity, perform weapon-damage against target, combat (action: advance) — assert output includes participant name, weapon used, damage roll transparency, target HP change. combat (action: advance) with no prior mutations — assert reports participant took no action. | REQ-043 |
...
| T176  | Automated | Extended error category semantics: call `condition (action: apply)` with an already-active condition — assert `[ERROR] [RULE_VIOLATION]` citing the ruleset anchor. Call a tool for a waived subsystem — assert `[ERROR] [UNIMPLEMENTED]` with the waiver reference. | REQ-002a |
...
| T225  | Automated | Section token discoverability: invoke `session (action: health)` — assert `section_tokens` array with token, group, and has_content fields. Invoke `help` with query `"briefing"` — assert valid token set enumerated. Invoke `help` with query `"section ordering"` — assert valid token set enumerated. Invoke `session (action: briefing_order)` with an unknown token — assert `[INVALID_INPUT]` with valid tokens enumerated matching `spec_health.section_tokens` exactly. | REQ-186, REQ-082 |
...
| T241  | Automated | World-model CRUD: create a room via `world (action: create_room, "Vault", "A stone chamber.")`. Create a thing via `world (action: create_thing, "crown", {location: "Vault", fixed: true})`. Create an exit via `world (action: create_exit, "north", "Vault", "Gallery")`. Assert reverse exit created. Assert `command("look")` in the Vault shows the crown. Assert `command("take crown")` returns rule-violation (fixed). Delete room via `world (action: remove_room, "Vault")` — assert crown and exits removed. Assert audit log records all mutations. Assert undo restores deleted room with contents. | REQ-198 |
...
| T246  | Automated | Combat-init guard: call `combat (action: init)` with valid participants — assert `[OK]` and combat is active. Call `combat (action: init)` again — assert `[STATE_CONFLICT]` with the message "Combat already active — call `combat (action: end)` first." Assert the active combat's round and turn order are unchanged by the rejected call. | REQ-203 |
| T247  | Automated | Combat participant validation: call `combat (action: init, participants=["nonexistent"])` with no entities imported — assert `[NOT_FOUND]` enumerating "nonexistent" and listing valid entity/NPC IDs; assert no combat state is created; assert `session (action: recap)` reports no pending confrontation. Call `combat (action: init)` with a mix of valid and invalid participant IDs — assert `[NOT_FOUND]` enumerating only the invalid IDs; turn-order construction does not begin. | REQ-204 |
| T248  | Automated | Mid-combat participant changes: during active combat with participants ["hero", "goblin"], call `combat (action: add_participant, "wizard")` — assert wizard inserted after hero in turn order. Call `combat (action: remove_participant, "goblin")` — assert goblin removed and pointer advances if goblin was current. Remove last participant from a 1-participant combat — assert auto-`combat (action: end)` with outcome "All participants removed." Assert undo reverts the participant change. Assert Player badge returns `[FORBIDDEN]`. | REQ-205 |
| T249  | Automated | Combat-round condition expiry: apply a condition with `rounds: 1` to a participant, call `combat (action: advance)` once — assert condition removed after turn and audit log contains `[condition-expired]`. Apply condition with `rounds: 0` — assert no decrement. Apply condition with no `rounds` field — assert no auto-expiry. Apply condition with `rounds: 2` — assert decrements to 1 after first `combat (action: advance)` and expires after second. | REQ-206 |
...
| T254  | Automated | Weighted table result: invoke `ruleset (action: roll, "wand_of_wonder", seed="42")` with a fixture table defining 1d100 → { 1-10: "Fireball", 11-20: "Stinking Cloud" }. Assert output includes dice notation, individual die face, matched range, and result text. Assert repeat with same seed produces identical result. Assert roll falling outside all ranges returns `[WARNING]` with raw roll and "no range matched" message. Assert table with interleaved lookup/dice-range rows is classified as generation-only. | REQ-213 |
| T255  | Automated | Table classification: invoke `ruleset (action: roll)` with a lookup-table name — assert the lookup table is not accessible through `ruleset (action: roll)`. Assert `ruleset (action: roll).table` enum contains only generation tables; assert no lookup table appears in the enum; assert the D&D 5e build includes trinkets, madness tables, and at least one equipment/spell d100 table; assert a fixture ruleset with zero generation tables returns `[NOT_FOUND]` with a "no tables" message. | REQ-214 |
| T256  | Automated | Table content extraction: assert `spec_health` reports `generation_tables` count ≥ 6 for D&D 5e; assert each entry has dice_expression, ranges, and source_anchor; assert `ruleset (action: roll, "trinkets", seed="42")` returns a valid trinket from the SRD trinket list, not a bare d100 number. | REQ-215 |
...
| T258  | Automated | Condition tools: apply a condition — assert `[OK]` and condition appears on `character (action: sheet)` and `badge_briefing`. Reapply same condition — assert `[WARNING]` with "Condition already active." and no duplicate in state. Remove condition — assert `[OK]`. Remove absent condition — assert `[WARNING]` with "Condition not present." Apply unknown condition — assert `[INVALID_INPUT]` with valid conditions enumerated. Player applies condition to another player's entity — assert `[FORBIDDEN]` with target entity ID. Apply condition with `rounds: 1` — assert GM `badge_briefing` shows the round count, Player `badge_briefing` does not. | REQ-217 |
| T259  | Automated | Ruleset-free build mode: start a build with B1="none". Assert the builder records ruleset-free mode in DECISIONS.md (1). Assert no chunked reading or extraction occurs. Assert the viability pre-check is skipped with "ruleset-free mode" recorded. Assert G0a passes with "no ruleset — skipped." Assert the server's `tools/list` contains all REQ-020 infrastructure categories. Assert no canonical lookup tools are registered (waived under REQ-013). Assert `ruleset (action: search)` returns empty results with "no ruleset indexed." Assert `ruleset (action: roll)` returns content-absent message. Assert `spec_health` reports the sentinel ruleset hash and zero indexed items. Assert `intro` prompt is ≤300 words, identifies the server by name, includes "world-model-only" notice, and ends with four concrete next actions (one of which references world-model parser commands). Assert Phase 1 and Phase 2 convergence metrics record `ruleset-free` zero-case dispositions. Assert H1 passes with "ruleset-free" entry. Assert H10 skips confidence with "ruleset-free" annotation. | REQ-218, REQ-063, REQ-020 |
| T260  | Automated | Ruleset-free entity creation: with server in ruleset-free mode, call `character (action: create, "Fen", description="tall, scarred", voice="clipped, wary", background="former guard", goals="find the Crown")`. Assert roster entry created with name and personality fields — no stats, no HP, no equipment. Assert `character (action: sheet, "fen")` displays name and personality fields with no stat block. Assert `character (action: import, "fen")` succeeds. Assert `combat (action: init, participants=["fen"], dangers=[{"name":"statue"}])` succeeds — Fen receives a turn in the order and auto-advances with `[auto]`. Assert `command (action: suggest, "fight")` returns empty list. Assert `entity://fen/personality` returns the populated fields. | REQ-219, REQ-077 |
| T261  | Manual   | World-model fixture replay: build a ruleset-free server and replay the Appendix W.3 golden transcript. Assert every interaction produces the expected prefix and tool name. Assert parser commands (look, go, take, open) resolve correctly against the world model. Assert badge gating — `combat (action: init)` blocks under Player badge. Assert undo restores item position. Assert countdown lifecycle. Assert lore triggers on scene transition containing keyword. Assert `novel (action: end)` confirmation workflow. Assert Appendix W.4 contracts are all exercised. | REQ-001, REQ-032, REQ-041, REQ-042, REQ-055, REQ-072, REQ-073, REQ-092, REQ-196, REQ-198, REQ-199, REQ-201 |
| T262  | Automated | Narrative POV directive: import two entities into a Novel, call `character (action: set_active, "character_01")`, assert `badge_briefing` includes a POV directive naming character_01 with narrative instruction and personality fields. Call `character (action: set_active, "character_02")` — assert directive updates to character_02. Remove all entities — assert omniscient empty-state marker. Assert the POV directive is present in the decision-critical group before entity listing. Assert the directive is never truncated under a tight briefing budget. | REQ-220 |
| T265  | Automated | POV omniscient mode: import two entities into a Novel. Call `character (action: set_active, "char_01", pov="omniscient")` — assert `badge_briefing` shows omniscient marker with char_01 as active entity. Call `character (action: set_active, "char_02")` — assert omniscient mode preserved with char_02. Call `character (action: set_active, "char_02", pov="character")` — assert character-locked POV for char_02. Assert Novel-scoped persistence: restart server, POV mode survives. | REQ-223 |
| T268  | Automated | Pause/resume context: call `novel (action: save_context, current_scene="tavern brawl", short_term_plans="Guards arrive in 2 rounds")` — assert `novel (action: get_context)` returns both fields. Restart server, resume same Novel — assert `intro` prompt includes GM context summary. Call `novel (action: end)` — assert `gm_context` cleared. Assert `novel (action: save_context)` auto-captures faction clock states and NPC dispositions. | REQ-232 |
| T269  | Automated | Factions: call `faction (action: create, "Merchant Guild", "Controls trade routes", goals=["Expand to East Dock"])` — assert faction created with `clock_type: faction` countdown. Assert `faction://<id>` returns faction with clock position. Call `countdown (action: advance)` on faction clock — assert faction display updates. Assert faction clock advances on scene transition. Call `faction (action: remove)` — assert faction and clock removed. | REQ-233 |
| T270  | Automated | Relationships: call `relationship (action: set, "pc_1", "npc_guard", "suspicious", value=3)` — assert `relationship (action: get, "pc_1")` includes the entry. Assert `character (action: sheet, "pc_1")` shows "Relationships: Guard (suspicious)." Call `relationship (action: set, "pc_1", "npc_guard", "ally")` — assert `badge_briefing` prompts GM to consider lore entry. Assert relationships saved in pause context. | REQ-236 |
| T271  | Automated | Clock types: create a `racing` clock pair with `opposes` — assert first to full wins, simultaneous complete produces tie. Create a `linked` clock chain — assert child clock triggers on parent completion. Create a `tug_of_war` clock — assert `retreat_countdown` removes ticks; retreat to zero does not trigger. Create a `mission` clock — assert it auto-decrements on `novel (action: resume)`. | REQ-073 |
| T272  | Automated | Session notation: call `session (action: recap, format="lonelog")` — assert output in Lonelog notation (`###` scene headers, `@` actions, `=>` outcomes). Call `session (action: compress, format="lonelog")` — assert compressed Lonelog entries. Assert audit entries contain optional `notation` field. Call `session (action: recap, format="markdown")` — assert current behavior unchanged. | REQ-072 |
| T273  | Automated | Player choices: call `scene (action: choices, "The goon blocks your path.", [{id:"talk", label:"Talk", description:"Persuade him"}, {id:"fight", label:"Fight", description:"Start combat"}])` — assert returns `[NEED_INPUT]` with two options. Call `respond(decision, "fight")` — assert `[choice]` audit entry and matching countdown advances. Call with `allow_freeform=true` — assert freeform text stored in audit entry. | REQ-235 |
| T274  | Automated | Secrets and knowledge: call `lore (action: set_secret, "confession", "The butler killed Lord Ashworth")` — assert GM-only lore entry created. Call `lore (action: reveal, "confession", "pc_detective")` — assert `character (action: sheet, "pc_detective")` includes "Known Information" section. Call `lore (action: knowledge, "pc_detective")` — assert returns the secret. Call `lore (action: knowledge, "pc_guard")` — assert does not return the secret. | REQ-234 |
| T275  | Automated | Session segmentation: run two sessions with different `TTRPG_SESSION_ID` values — assert audit log contains two `[session-boundary]` markers with session IDs and timestamps. Call `session (action: recap, session_id="s1")` — assert returns only entries from session s1. Call `session (action: recap, session_id="s2")` — assert returns only entries from session s2. Call `session (action: recap)` with no session_id — assert returns all entries. Call `session (action: health)` — assert per-session metrics array includes entry counts, timespans, and combat rounds for both sessions. | REQ-237 |
| T276  | Automated | Backup rotation: set `TTRPG_NOVEL_BACKUP_COUNT=3` — after 10 mutations, assert three rotated backup files exist (`.bak.1`, `.bak.2`, `.bak.3`) with descending modification times. Corrupt the primary `.json` file and `.bak.1` — restart the server, assert it restores from `.bak.2` and the audit log contains a `[restored-from-backup]` entry naming backup index 2. Call `novel (action: end)` — assert all backup files and the primary are moved to `.trash/`. | REQ-238 |
| T277  | Automated | Audit log compaction: with `TTRPG_AUDIT_RETENTION_SESSIONS=1`, run two sessions — assert audit log has entries for both. Call `session (action: compress)` — assert `[NEED_INPUT]` confirmation prompt. Confirm with `respond(decision, "yes")` — assert session 1 entries are gone from live audit log, `audit://novel/archive` contains session 1 summary with timespan, entry_count, confrontations, and significant_rolls. Call `session (action: recap, session_id="s1")` — assert returns the summary from archive. Call `session (action: recap)` with no session_id — assert returns only session 2 entries. Call `session (action: compress, sessions=2)` — assert prompt to retain both sessions. Player badge `session (action: compress)` returns `[FORBIDDEN]`. | REQ-239 |
| T278  | Automated | Clone Novel: call `novel (action: clone, "my-novel", "my-novel-fork")` — assert new Novel created at `novels/my-novel-fork.json`, `spec_health` lists both Novels, source Novel's active flag unchanged. Set Fate point and momentum state in the source before cloning — assert the clone preserves both (REQ-434–443). Give the source NPC a populated mind — assert the clone preserves the NPC mind object (REQ-075f). Mutate the clone (add NPC) — assert source Novel unaffected. Call `novel (action: clone, "my-novel", "my-novel-fork")` again — assert `[STATE_CONFLICT]`. Call `novel (action: clone, "my-novel", "trimmed", trim_audit_sessions=2)` — assert cloned audit log contains only 2 most recent sessions. Player badge `novel (action: clone)` returns `[FORBIDDEN]`. | REQ-240 |
| T279  | Automated | Checkpoints: call `novel (action: checkpoint_set, "before-ritual")` — assert checkpoint created, `novel (action: checkpoint_list)` returns `{label: "before-ritual", ...}`. Set Fate point state before the checkpoint — after restore, assert the base-capability state is reverted with the rest of the snapshot (REQ-434–443). Give an NPC a populated mind before the checkpoint — after restore, assert the NPC mind is reverted with the snapshot (REQ-075f). Perform 5 mutations. Call `novel (action: checkpoint_restore, "before-ritual")` — assert `[NEED_INPUT]`, confirm `yes`, assert all 5 mutations reversed. Call `novel (action: checkpoint_remove, "before-ritual")` — assert `novel (action: checkpoint_list)` is empty. Set `TTRPG_MAX_CHECKPOINTS=1`, create two checkpoints — assert oldest discarded. Call `novel (action: end)` — assert checkpoints cleared. `novel (action: export, "json", include_checkpoints=true)` — assert `checkpoints` key present. Player badge returns `[FORBIDDEN]`. | REQ-241 |
| T280  | Automated | Notes: call `note (action: set, "twist", "The king is the dragon")` — assert stored with default `game_master` scope. Call `note (action: set, "clue", "The key is in the clock", "player")` — assert stored with `player` scope. Call `note (action: list)` under Game Master badge — assert returns both notes with their badge_scope values. Switch to Player badge — assert `note (action: list)` returns only the `player`-scoped note; `notes://twist` returns `[FORBIDDEN]`; `notes://clue` returns full content. Assert Player `badge_briefing` `notes` section shows only the player-scoped note. Assert GM `badge_briefing` `notes` section shows both notes. Call `note (action: set, "gm_only", "secret", "game_master")` under Player badge — assert `[FORBIDDEN]`. Call `note (action: remove, "twist")` — assert note removed. `novel (action: export, "json")` — assert `notes` key present with `{content, badge_scope}` objects. `novel (action: end)` — assert notes cleared. | REQ-242 |
| T281  | Automated | Novel interchange validation: call `novel (action: export, "json")` — assert `manifest` object present with all declared fields (novel_format_version, server_spec_version, ruleset_hash, builder_implementation, adventure_module_slugs, adventures_embedded, property_groups_present, waiver_dependent_mechanics). Call `novel (action: export, "json", "lore")` — assert only `format_version`, `manifest`, `lore`, and `novel_metadata` keys present. Call `novel (action: export, "json", "gm_context")` — assert only `format_version`, `manifest`, `gm_context`, and `novel_metadata` present. Export with full scope, introduce a broken NPC reference in a lore trigger via manual JSON editing — call `novel (action: import, modified_data, "dry-run")` — assert validation failure reporting the broken reference with item path. Call `novel (action: import, modified_data, "replace")` — assert `[WARNING]` with broken reference enumerated but import succeeds. Call `novel (action: import, modified_data, "replace", strict=true)` — assert `[ERROR] [STATE_CONFLICT]` with failure list, state unchanged. Call `novel (action: import, modified_data, "dry-run", strict=true)` — assert `isError: false` with failure report. | REQ-096 |
| T282  | Automated | Story journal: call `story (action: record, "moment", "The ferryman told a story...")` during an active scene with entity "eira" on scene "river-crossing" — assert entry appended to `story_journal` array in Novel JSON with `type: "moment"`, `entry`, `timestamp` (ISO 8601), `scene_anchor: "river-crossing"`, and `entity_ids: ["eira"]`. Call `undo` — assert story journal entry persists (not undone). Call `novel (action: export, "json")` — assert `story_journal` key present with one entry. Call `story (action: record, "moment", "...")` under Player badge — assert `[FORBIDDEN]`. Call `story (action: record, "invalid_type", "...")` — assert `[ERROR] [INVALID_INPUT]` with valid type enumeration. Invoke `session (action: recap)` — assert `story_entries` field present as array with at most 10 most recent entries. Invoke `badge_briefing` with scene set to "river-crossing" and entity "eira" active — assert `story` section token present containing the ferryman entry. Call `novel (action: end)` — assert `story_journal` cleared. | REQ-246 |
...
| T284  | Automated | Adventure scene waypoint: load an adventure with a populated structural index. Call `scene (action: set, adventure_scene="Some Scene Heading")` — assert `badge_briefing` surfaces the adventure scene description labeled with the active adventure slug and scene heading. Assert adjacent scenes (previous and next in the structural index) are listed in `badge_briefing`. Assert the GM's free-text `description` remains independent and visible alongside the adventure scene. Call `scene (action: set, adventure_scene="")` — assert waypoint cleared, adventure scene absent from briefing. Call `scene (action: set, adventure_scene="nonexistent-heading")` — assert `[ERROR] [NOT_FOUND]` with nearby scene names enumerated. Assert changing the waypoint fires a scene transition hook (REQ-125). Switch to Player badge — assert adventure scene visible in briefing passively, `scene (action: set)` with `adventure_scene` returns `[FORBIDDEN]`. | REQ-250, REQ-125, REQ-032 |
| T285  | Automated | Adventure overview resource: load an adventure with populated structural extraction. Call `resources/read` on `adventure://<slug>/overview` — assert premise, NPC list with role descriptions, location list, faction descriptions, and scene count present. Switch to Player badge — assert GM-only sections (NPCs marked Keeper-only) hidden; premise and shared locations visible. Call `adventure (action: load)` on a module with empty structural extraction — assert `[WARNING]` with "No structured overview available." | REQ-248, REQ-079, REQ-032 |
...
| T311 | Automated | Generation intent guard: call `adventure (action: generate, "create an adversary capable of defeating Data")` — assert `[WARNING]` listing the guard concern: power-inversion intent, fabricated mechanics required. Assert no adventure content is produced. Call `adventure (action: generate, "!force create an adversary capable of defeating Data")` — assert adventure scaffold generated, `[generation-guard-overridden]` audit entry recorded. Call with a ruleset that defines challenge ratings — assert `adventure (action: generate_encounter, "dragon fight")` caps danger difficulty against party level and warns on exceedance. Assert `badge_briefing` under GM badge includes guard-fire advisory when a guard fired in the current session. | REQ-251 |
| T312 | Automated | Narrative fast-forward: call `scene (action: set, "The castle", fast_forward={interval:"three days travel", changes:[{npc_id:"guard_1", location:"castle gate"}]})` — assert audit entry `[fast-forward]` recorded with interval, countdown adjustments, and NPC update. Assert narrative countdown advanced by 3-day-equivalent ticks. Assert guard_1 location updated. Assert `undo` restores pre-fast-forward state. Assert fast-forward with `skip_countdowns: true` preserves countdown positions. Assert fast-forward + `skip_transition_hook` returns `[ERROR] [INVALID_INPUT]`. Assert Player badge returns `[FORBIDDEN]`. | REQ-252, REQ-073, REQ-125 |
| T313 | Automated | Tool-output verbosity control: call `lookup_spell("fireball", terse=true)` — assert output includes spell name, level, and damage die; assert verbal/somatic/material component text absent. Set `character (action: signal, "detail", "terse")` — assert subsequent `ruleset (action: search, "grapple")` returns most relevant sentence only. Assert `combat (action: advance)` under terse mode returns participant name + action result only, no full roll transparency. Assert `detail=normal` restores full output. Assert per-call `terse: true` overrides session detail signal. Assert `detail=rich` returns full descriptions with lore trigger notifications. | REQ-253, REQ-196, REQ-069 |
| T314 | Automated | Boundary signal propagation: set boundary "spiders" — assert recorded. GM badge_briefing — assert Boundaries advisory section with "spiders" and "Do not narrate, imply, or introduce content" directive, positioned before scene state. Call `scene (action: set, "a cavern full of spiders")` — assert `[WARNING]` boundary collision identified, operation proceeds. Call `npc (action: create, "spider queen")` — assert `[WARNING]`. Call `scene (action: set, "a sunny meadow")` — assert `[OK]` no warning. Player badge — assert Boundaries absent from briefing. Remove boundary — section absent from subsequent briefing. Set two boundaries ("spiders", "drowning") — assert both listed in advisory. | REQ-255, REQ-069, REQ-128 |
...
| T315 | Automated | Rename Novel: create a Novel with slug "my-novel". Call `novel (action: rename, "my-novel", "my-novel-v2")` — assert `[OK]`, novel now accessible under new slug. Call `novel (action: resume, "my-novel")` — assert `[STATE_CONFLICT]`. Call `novel (action: resume, "my-novel-v2")` — assert Novel state restored. Assert renamed Novel metadata reflects new slug in `spec_health`. Assert rename to existing slug returns `[STATE_CONFLICT]`. Assert rename of non-existent Novel returns `[NOT_FOUND]`. Assert Player badge returns `[FORBIDDEN]`. | REQ-256 |
| T316 | Automated | List Novels: create two Novels (A and B). Call `novel (action: list)` — assert both slugs listed with metadata (name, last_played, entity_count, active). Assert active Novel marked. Call `novel (action: end)` on A — assert A absent from listing. Call `novel (action: list)` on a clean server — assert empty list with empty-state marker. Assert `novel (action: list)` is callable regardless of badge — both Player and Game Master see same listing. | REQ-257 |
| T317 | Automated | Novel info: create a Novel with entities, NPCs, lore entries, and an active adventure. Call `novel (action: info)` — assert output includes slug, name, created_at, last_played, entity_count, adventure_module, session_count, and total_combat_rounds. Assert `novel (action: info, slug="my-novel")` with a non-active Novel returns same fields. Assert `novel (action: info, slug="nonexistent")` returns `[NOT_FOUND]`. Assert `novel (action: info, format="json")` returns all metadata fields in structured format matching `spec_health` novel fields. | REQ-258 |
| T318 | Automated | Update Novel description: create a Novel with name "Original Name". Call `update_novel(description="A tale of adventure")` — assert `[OK]`, description stored. Call `novel (action: info)` — assert description field present. Call `badge_briefing` — assert description appears under novel token. Call `update_novel(name="New Name", description="Revised")` — assert both fields updated. Call `update_novel()` with no fields — assert `[ERROR] [INVALID_INPUT]`. Assert Player badge returns `[FORBIDDEN]`. Assert `novel (action: end)` then `novel (action: resume)` — assert description persists. | REQ-259 |
| T319 | Automated | Granular synthesis activation: build with synthesis active across 7 modules. Call `synthesis (action: toggle, "lore_templates", true)` — assert `[OK]`, module activated and items appear in `badge_briefing`. Call `synthesis (action: toggle, "lore_templates", false)` — assert module deactivated, items absent from `badge_briefing`. Call `synthesis (action: toggle, "unknown_module", true)` — assert `[INVALID_INPUT]` with valid module names enumerated. Assert `spec_health` reports per-module activation status. Assert activation is Novel-scoped — restart restores module activation states. Assert Player badge returns `[FORBIDDEN]` on activation/deactivation of non-`[player]` items. Assert Player can call `synthesis (action: deactivate)` on own `[player]` items — item hidden from player briefing. Assert Player calling `synthesis (action: activate)` on a `[ruleset]` item returns `[FORBIDDEN]`. Assert `synthesis (action: revert)` resets all modules to inert. | REQ-260 |
| T320 | Automated | Player synthesis: build with synthesis active. Call `synthesis (action: player_add, "action_patterns", "feint-suggestion", "When I feint, suggest deception check")` — assert item appears in Player `command (action: suggest)` output and in GM `badge_briefing` (default shared scope). Call `synthesis (action: player_add, "voice_examples", "growl", "Get away!", [], "player")` — assert item visible to Player badge, absent from GM badge. Call `synthesis (action: player_list)` — assert both items listed with module, key, preview, and scope. Call `synthesis (action: player_remove, "action_patterns", "feint-suggestion")` — assert item removed. Call `synthesis (action: player_remove)` on a Tier 1 `[ruleset]` item — assert `[RULE_VIOLATION]`. Call `synthesis (action: list)` as GM — assert `[player]` items visible with source tag. GM attempts `synthesis (action: player_add)` — assert `[FORBIDDEN]`. Assert player items survive server restart. Assert `synthesis (action: revert)` does not remove `[player]` items. Assert per-module cap of 15 enforced. | REQ-261 |
| T321 | Automated | Novel synthesis: create a Novel with populated NPCs, lore entries, story journal entries, and faction state. Call `synthesis (action: run)` — assert internal items carry the `[supplementary]` tag with `novel://<slug>/` source URIs and appear in `synthesis (action: list)`. Call `synthesis (action: revert)` — assert `[supplementary]` items are removed. Call `novel (action: end)` — assert synthesis items are discarded with the Novel JSON. Assert Player may deactivate internal items scoped `shared` or `player` via `synthesis (action: deactivate)`. | REQ-262, REQ-103, REQ-265, REQ-260 |
| T322 | Automated | Novel synthesis tool: create a Novel with an NPC possessing personality fields (description, voice, goals). Call `synthesis (action: run)` — assert voice example items produced with `source: novel://<slug>/npc/<npc_id>`. Call again with no state changes — assert up-to-date message with ISO 8601 timestamp. Call `synthesis (action: run, force=true)` — assert re-synthesis regardless of staleness. Assert Player badge returns `[FORBIDDEN]`. Assert items are inert (inactive) by default — GM must activate via `synthesis (action: activate)`. | REQ-263 |
| T323 | Automated | Novel synthesis auto-trigger: set `TTRPG_SYNTHESIS_AUTO_TRIGGER=on_session_start`. Change `TTRPG_SESSION_ID` — assert `synthesis (action: run)` triggers, items appear in `synthesis (action: list)` with `activated: false`. Set to `off` — assert no auto-trigger on session change. Set to `on_scene_change` — call `scene (action: set, "new scene")` — assert synthesis triggers. Assert `spec_health` shows `synthesis_auto_trigger: <threshold>`. Create a ruleset-free Novel with no state — assert synthesis produces `[supplementary] [empty — no state]` markers. | REQ-264 |
| T324 | Automated | Novel synthesis removal: create a Novel with `[supplementary]` synthesis items. Call `synthesis (action: revert)` — assert `synthesis (action: list)` returns zero `[supplementary]` items. Assert Tier 1 `[ruleset]`/`[vendor]` items unchanged. Call `synthesis (action: revert)` again — assert idempotent, no error. Assert Player badge returns `[FORBIDDEN]`. | REQ-265, REQ-103 |
| T325 | Automated | Novel synthesis confidence: create a Novel with an NPC carrying explicit personality text. Call `synthesis (action: run)` — assert voice example item carries `[supplementary] [MEDIUM]`. Create a story journal entry and call synthesis — assert theme-detection item carries `[supplementary] [LOW]`. Edit the NPC personality, call synthesis — assert `collected_at` timestamp updated. Assert `MEDIUM` items always correspond to explicit-field sources; `LOW` items always correspond to inference sources. | REQ-266 |
| T326 | Automated | Synthesis in badge_briefing: create a Novel with active synthesis items. Call `badge_briefing` under GM badge — assert synthesis items appear under respective sections tagged `[supplementary]` alongside `[ruleset]`, `[vendor]`, and `[player]` items. Switch to Player badge — assert only synthesis items with scope `shared` or `player` visible; `game_master`-scoped items absent. Call `synthesis (action: deactivate)` on a synthesis item scoped `shared` — assert item hidden from Player badge. Call `synthesis (action: revert)` — assert no synthesis section in `badge_briefing` (no empty-section marker). | REQ-265, REQ-032, REQ-260 |
| T327 | Automated | Synthesis in dashboard: create a Novel with synthesis items in voice_examples and lore_templates modules. Call `resources/read` on `synthesis://status` — assert synthesis column present in per-module table alongside Ruleset Wisdom column. Assert non-zero counts for voice_examples and lore_templates in the synthesis column. Call `session (action: health)` — assert `synthesis_status` with per-module activated/total counts and `synthesis_last_run` timestamp. Call `synthesis (action: revert)` — assert synthesis column shows zero counts. | REQ-266, REQ-230 |
| T334 | Automated | Server notes: call `note (action: set_server, "campaign-bible", "The old gods were banished to the outer dark")` — assert stored. Call `note (action: list_server)` — assert returns `{key: "campaign-bible", preview: "The old gods were banished to the outer dark"}`. Call `server-notes://campaign-bible` — assert full content returned. Restart server — assert server note survives. Create and `novel (action: end)` — assert server notes persist. Switch to Player badge — assert `note (action: set_server)`, `note (action: remove_server)`, `note (action: list_server)`, and `server-notes://<key>` all return `[FORBIDDEN]`. Call `session (action: health)` — assert `server_notes` count present. Call `note (action: remove_server, "campaign-bible")` — assert `note (action: list_server)` is empty. | REQ-285 |
...
| T349 | Automated | Observer mode: call `set_badge("observer")` — assert returns `[OK] Active badge: observer — read-only spectator mode`. Call `scene (action: set, "test")` — assert `[FORBIDDEN]` with corrective action citing `set_badge`. Call `help()` — assert succeeds. Call `badge_briefing` — assert orientation includes dual-role instruction. Call `character (action: sheet, entity_id)` — assert succeeds. Switch to player badge then back to observer — assert `[FORBIDDEN]` still applies for mutating tools. | REQ-305 |
| T350 | Automated | Adjustable autonomy: call `scene (action: autonomy, {level: "full", confirmation: "auto", safety: "safe", creativity: "standard"})` — assert `[OK]`. Invoke `badge_briefing` — assert autonomy state visible. Set `level=mechanical_prompt, confirmation=prompt` — assert AI auto-narrates exploration but pauses via `scene (action: choices)` for combat actions; human responds via `respond`. Set `safety=safe` — assert lethal damage reduces HP to 1 and applies incapacitation instead of death. Set `safety=hardcore` — assert death permanent, no warnings. Set `creativity=chaotic` — assert AI makes unexpected, dramatic choices. Assert Player badge `scene (action: autonomy)` returns `[FORBIDDEN]`. Assert autonomy persists across Novel restart. | REQ-306 |
| T351 | Automated | Entity presence: call `scene (action: set, "Dark corridor", characters_present=["rogue_01"])` — assert `party://current` shows rogue present, wizard not present with `[not present]`. Call `scene (action: presence, ["wizard_01"], "Camp")` — assert scene description unchanged, wizard present. Call `scene (action: presence, [])` — assert all entities not present. Entity listing in `badge_briefing` — assert `[not present]` markers and `last_location` fields. `character (action: set_active)` to non-present entity — assert no error, `knowledge_state` renders "Entity not present" marker. | REQ-307 |
| T352 | Automated | Knowledge gating by presence: set scene to corridor with rogue only present. Call `lore (action: reveal, "floor_trap", "rogue_01")` — assert rogue's `knowledge_state` includes trap. Set scene to camp with wizard only present — assert wizard's `knowledge_state` does NOT include trap. Reunite party (`characters_present` includes all). Assert rogue's trap knowledge retained, wizard's still excludes trap. Call `lore (action: reveal, "floor_trap", "wizard_01")` — assert wizard now knows trap. | REQ-308 |
| T353 | Automated | World and narrative surface prominence: build with `TTRPG_WORLD_PROMINENCE=secondary` — assert World tools in secondary help category, world-model state folded into scene state in `badge_briefing`, `command (action: suggest, "go north")` returns no parser commands. Build with `TTRPG_WORLD_PROMINENCE=visible` — assert world-model and narrative tools in primary help categories, dedicated world-model state section in `badge_briefing` with empty-state marker when unpopulated, `command (action: suggest, "go north")` returns parser command alongside TTRPG tools. Build with `TTRPG_WORLD_PROMINENCE=prominent` — assert parser `command` is a top-level help entry, world CRUD tools in primary setup category, world-model state in decision-critical `badge_briefing` group, `command (action: suggest, "go north")` returns parser command before TTRPG tools. Assert ruleset-free build skips B12 and records no `TTRPG_WORLD_PROMINENCE` value. | REQ-309 |
...
| T303 | Automated | Two-tier synthesis model: build with synthesis active. Assert `synthesis (action: list)` returns ruleset-native `[ruleset]` items and synthesis `[supplementary]` items distinguished by source tag. Assert `synthesis (action: revert)` removes `[supplementary]` items only. Assert `[ruleset]` items survive `synthesis (action: revert)`. | REQ-227 |
| T304 | Automated | Synthesis consistency during spec-driven updates: perform a Minor spec-driven update that adds a tool surface. Assert the gap audit records the old surface was removed and synthesis items for that surface are flagged for GM review. Assert `badge_briefing` under GM badge includes advisory about removed surface. Assert `synthesis (action: revert)`+re-activation preserves existing items for unchanged surfaces. | REQ-228 |
| T305 | Automated | Adventure synthesis linkage: call `adventure (action: load)` on an adventure with synthesis data. Assert adventure-linked synthesis items appear in `synthesis (action: list)` tagged with the adventure slug. Assert `novel (action: end)` removes adventure-linked items. Assert `synthesis (action: revert)` does not remove adventure-linked items. | REQ-229 |
...
| T328 | Automated | Narrative orientation: call `session (action: recap)` after a session with scene changes. Assert `narrative_orientation` section includes recent plot beats, unresolved threads, and party state. Call `novel (action: end)` then `novel (action: create)` — assert `session (action: recap)` returns empty orientation. Assert orientation references lore entries that fired during the session. | REQ-279 |
...
| T336 | Automated | Knowledge-state section token: call `lore (action: reveal, "floor_trap", "rogue_01")`. Assert `badge_briefing` under GM badge includes `knowledge_state` section token listing the revealed secret. Set active entity to an entity not present in the current scene — assert section renders "[Entity not present in this scene]" marker. Assert Player badge sees only own-entity knowledge. Assert empty Novel renders the empty-state marker. | REQ-286 |
| T331 | Automated | Scene-state ledger: call `scene (action: set, "Tavern", "Merry tavern")`. Assert badge_briefing includes scene token with current location and description. Call `scene (action: set, "Forest", "Dark forest")` — assert prior scene pushed to ledger. Call `undo` — assert ledger restored to prior scene. Assert `session (action: recap)` includes scene transitions. | REQ-076 |
| T332 | Automated | NPC voice directive: call `character (action: personality, npc_id, voice="Gruff and impatient")` on an NPC. Assert `badge_briefing` under GM badge includes NPC voice directive with the personality fields. Assert the voice directive includes positive framing ("SHALL") and negative counsel ("should NOT") per REQ-282. Assert Player badge does not see GM-only voice directives. | REQ-282 |
...
| T335 | Automated | Vow tracking: call `vow (action: set, "vengeance", "Track down and defeat the brigand leader", entity_id)`. Assert `badge_briefing` includes Vows section with the vow description, target, and entity. Call `vow (action: resolve, "vengeance")` — assert vow marked resolved in briefing. Call `vow (action: forsake, "vengeance")` — assert vow marked forsaken with `[vow-forsaken]` marker. Assert `list_vows()` returns all active/forsaken vows. | REQ-289 |
| T337 | Automated | Oracle tool: call `scene (action: oracle, "Is the door locked?", "unlikely", seed="test")` — assert returns Yes/No with modifier annotation. Call `scene (action: oracle, "What's behind the door?", "50_50", seed="test")` — assert returns result. Assert deterministic output with same seed. Assert `scene (action: oracle, "...")` with likelihood omitted defaults to `50_50` and returns a result. | REQ-291 |
| T338 | Automated | Adventure catalog: call `adventure (action: list)` on a server with indexed adventure modules. Assert each entry includes slug, title, and ruleset compatibility. Call `adventure (action: list, filter="fantasy")` — assert filtered results. Assert `adventure (action: list)` on empty catalog returns empty-state marker. | REQ-292 |
| T339 | Automated | Genre declaration: call `novel (action: genre, "horror")` on an active Novel. Assert `novel (action: info)` reports genre. Assert `spec_health` includes genre field. Call `novel (action: genre, "fantasy")` — assert updated. Assert `novel (action: genre, "invalid_genre")` returns `[ERROR] [INVALID_INPUT]` with valid genres enumerated. | REQ-294 |
| T340 | Automated | Genre-filtered generation: set genre to "horror" on a Novel. Call `adventure (action: generate_encounter, "forest clearing")` — assert encounter elements include horror-appropriate tropes. Set genre to "fantasy" — assert encounter elements shift to fantasy-appropriate tropes. Assert `adventure (action: generate, "dungeon")` respects genre in adventure scaffold. | REQ-295 |
...
| T342 | Automated | Dynamic lore: call `lore (action: set, "tavern_secret", "The barkeep is a retired assassin", triggers=["tavern", "barkeep"])`. Assert `badge_briefing` includes lore item when scene description matches triggers. Call `lore (action: toggle, "tavern_secret")` to disable — assert item absent from briefing. Call `lore (action: toggle, "tavern_secret")` to enable — assert item returns. Call `lore (action: remove, "tavern_secret")` — assert item removed. | REQ-083 |
...
| T344 | Automated | Structured failure diagnostics: induce a G2 failure on combat (action: init) turn-order mismatch. Assert DECISIONS.md (5) contains a diagnostic record with gate name, failing test ID, REQ citation, expected turn order, actual turn order, and a diff. Assert resolution field transitions from pending to converged on fix. | REQ-300 |
...
| T366 | Automated | Codex: call `codex (action: set, "npc", "Blacksmith", {description: "Gruff, scarred", ac: 14, hp: 35}, "The village blacksmith", ["blacksmith", "village"])` — assert stored with default visibility `library`. Call `codex (action: set, "npc", "Fighter", {...}, visibility="shared")` — assert `shared` visibility. Call `codex (action: list, "npc")` under Player badge — assert only `shared` entries. Call `codex (action: list, "npc")` under Game Master badge — assert all entries. Call `codex (action: list, "npc")` with the Editor badge — assert all entries unfiltered. Call `codex (action: get, "blacksmith")` — assert full record with data payload. Restart server — assert codex entries survive. Create and `novel (action: end)` — assert codex entries persist. Call `codex (action: import, "blacksmith")` under Game Master badge into an active Novel — assert NPC created with stored fields. Call `codex (action: import, "fighter")` under Player badge (character kind, shared visibility) — assert character imported via `character (action: import)`. Call `codex (action: import, "blacksmith")` under Player badge — assert `[FORBIDDEN]`. Call `codex (action: set, ...)` under Player badge — assert `[FORBIDDEN]`. Call `codex (action: import, "my-adventure")` with kind `adventure` into an active Novel — assert world-model, NPCs, factions, lore, and synthesis linkages populated. Call `codex (action: capture, "npc", "blacksmith")` from within a Novel — assert `source_novel` field populated. Call `codex (action: capture, "adventure")` from a Novel with adventure content — assert Codex entry created with `source: captured:<slug>`. Call `codex (action: capture, "adventure")` from a Novel with no adventure content — assert `[STATE_CONFLICT]`. Call `codex (action: import, "blacksmith")` with no Novel active — assert `[STATE_CONFLICT]`. Call `session (action: health)` — assert `codex` key reports counts partitioned by kind. Call `codex (action: delete, "blacksmith")` — assert removed. Call `undo` — assert entry restored. | REQ-321 |
| T367 | Automated | Adventure generation codex: call `adventure (action: generate, "The goblin king demands tribute")` with active Novel — assert scaffold at `adventure://generated/<anchor>`. Call `adventure (action: generate, "The dragon hoard", target="codex")` with no Novel active — assert `[OK]`. Call `codex (action: list, "adventure")` — assert entry present with `source: generated`. Call `codex (action: get, "<slug>")` — assert full adventure data payload including title, premise, overview, hook, locations, npc_suggestions, encounter_seeds, genre_tags, sections. Restart server — assert Codex entry survives. Call `adventure (action: generate, "The dragon hoard", target="codex")` again — assert entry replaced (same slug). | REQ-090 |
| T368 | Automated | Adventure loading codex: call `adventure (action: load, "tomb-of-the-serpent-king")` with active Novel — assert world-model populated and NPCs created. Call `adventure (action: load, "tomb-of-the-serpent-king", target="codex")` with no Novel active — assert `[OK]`. Call `codex (action: list, "adventure")` — assert entry present with `source: loaded:tomb-of-the-serpent-king`. Call `codex (action: get, "<slug>")` — assert full sections payload. Restart server — assert Codex entry survives. | REQ-079 |
| T369 | Automated | Vow-countdown coupling: call `vow (action: set, "Find Crown", "Recover the lost Crown of Alara", parties=["pc_1"], difficulty="dangerous", scope="shared")` — assert `badge_briefing` narrative_threads includes countdown suggestion. Accept suggestion via decision workflow — assert 20-tick `clock_type: mission` countdown created with name `vow:Find Crown`. Call `vow (action: milestone, "Find Crown")` — assert both milestone counter and countdown advance by one tick. Advance countdown to fill — assert vow becomes eligible for `vow (action: resolve)`. Call `vow (action: resolve, "Find Crown", "Found it", "Kingdom restored")` — assert countdown removed. Call `vow (action: set, "Other Vow", "A minor task", parties=["pc_1"], difficulty="troublesome", scope="gm")` — decline countdown suggestion — assert vow functions with milestones-only (current behavior). | REQ-322 |
| T370 | Automated | command (action: resolve) tool: populate world model with rooms Entrance (exits: north → Hall), Hall (exits: south, north → Chapel, locked door north), Chapel. Call `command (action: resolve, "go north")` from Entrance — assert status `resolved`, room_context.name = "Hall". Call `command (action: resolve, "go north")` from Hall — assert status `blocked` with constraint `locked`. Create character with Knock spell — create override entry. Call `command (action: resolve, "go north")` from Hall as that character — assert override_hints includes Knock. Call `command (action: resolve, "go east")` where no east exit — assert `blocked` with constraint `exit`. Call `command (action: resolve)` under Player badge — assert `[FORBIDDEN]`. World model unpopulated — assert `no_world_model`. Assert tool appears in `tools/list`. Assert `help("command (action: resolve)")` returns usage. | REQ-323 |
...
| T372 | Automated | Constraint override catalog: build with constraint overrides present. Call `resources/read` on `constraints://active` as Game Master — assert all overrides returned with type, name, source, prerequisites. Call as Player with active entity having 1 override — assert only that override returned. Call `command (action: resolve)` against a locked door with Knock available — assert error response includes `Hint: Knock (1 slot remaining) can open it.` Call with no override available — assert no hint. `spec_health` reports `constraint_override_counts` by type. | REQ-325 |
| T373 | Automated | Scene-world coupling: create world model with room "Throne Room" (exits: north, south; things: throne, chandelier). Call `scene (action: set, "The royal chamber", location="Throne Room")` — assert `scene://current` includes `room_id`, `room_name: "Throne Room"`. Call `command (action: resolve, "look")` as GM — assert exits and contained things composable. Call `scene (action: set, "The void", location="Nowhere")` — assert `location` is free-text, no `room_id`. Call `scene (action: set, "The room", location="Throne")` — assert fuzzy match resolves to "Throne Room". Call `undo` — assert scene state restored. | REQ-326 |
| T374 | Automated | NPC-world coupling: create world model with rooms Forge and Inn. Call `npc (action: create, "Blacksmith", location="Forge")` — assert NPC registered in Forge room. Call `command (action: resolve, "look")` from Forge — assert Blacksmith listed in room_context.present_npcs. Call `npc (action: update, "blacksmith", location="Inn")` — assert Blacksmith no longer in Forge; `command (action: resolve, "look")` from Inn lists Blacksmith. Call `npc (action: create, "Stranger", location="The Void")` — assert no room match; NPC carries free-text location only, not registered in any room. Assert `characters_present` on `scene (action: set)` still governs badge_briefing NPC presence. | REQ-327 |
| T375 | Automated | Lore-world coupling: create world model with room Chapel, thing altar_01. Call `lore (action: set, "altar_secret", "The altar hums with dark power", world_target="altar_01")`. Call `command (action: resolve, "examine altar")` from Chapel — assert lore triggers with `[world]` tag. Call `command (action: resolve, "look")` from Chapel (no target interaction) — assert lore does NOT trigger on keyword match alone. Call `lore (action: set, "room_rumor", "The chapel feels wrong", triggers=["chapel"], world_target="chapel")`. Call `command (action: resolve, "look")` from Chapel with "chapel" keyword in scene — assert lore triggers on keyword match. Call `command (action: resolve, "enter chapel")` — assert lore triggers on room entry. Call `lore (action: suggest)` — assert world-targeted lore returned when target reachable. | REQ-328 |
| T376 | Automated | Countdown-world coupling: create world model with room Guard Room. Call `countdown (action: set, "ambush", 3, type="narrative", triggers=["on_room_enter(guard_room)"])`. Call `command (action: resolve, "go north")` from adjacent room into Guard Room — assert countdown advances by one tick. Navigate into Guard Room three times — assert countdown fires and is removed. Call `countdown (action: set, "raid", 5, type="round", triggers=["on_room_enter(throne_room)", "on_thing_take(crown_01)"])`. Start combat — combat (action: advance) four times — assert countdown at 1 tick remaining. Navigate into Throne Room — assert countdown fires. Remove countdown — assert no further trigger fires. | REQ-329 |
| T377 | Automated | Knowledge-world coupling: create world model with rooms Entrance, Guard Room, Chapel. Call `command (action: resolve, "go north")` from Entrance to Guard Room as entity "rogue_01" — assert `knowledge_state` includes "Guard Room" under "Explored" with timestamp. Call `command (action: resolve, "go north")` from Guard Room to Chapel — assert Chapel added. Return to Guard Room — assert no duplicate. Call `scene (action: set, "Camp", characters_present=["rogue_01"])` — assert GM declaration overrides exploration presence. `knowledge_state` retains all prior exploration entries. Call `command (action: resolve, "look")` in Guard Room — assert NPCs seen added to knowledge. | REQ-330 |
| T378 | Automated | Story journal-world coupling: create world model with room Library. Call `scene (action: set, "The library", location="Library")` (scene-world coupled). Call `story (action: record, "moment", "Found the hidden map behind the bookshelf")` — assert entry auto-populates `room_id: "library"` and `scene_anchor` includes "Library". Call `story (action: list)` — assert entry shows room name. Call `scene (action: set, "The void", location="Nowhere")` (no match). Call `story (action: record, "moment", "Drifted through nothingness")` — assert `room_id` absent. Call `session (action: recap)` — assert "Library" in narrative_orientation for first entry, absent for second. | REQ-331 |
| T379 | Automated | Codex bootstrap: call `codex (action: set, "adventure", "Dragon Hoard", {title: "The Dragon Hoard", premise: "A dragon demands tribute", sections: {}})` to create a Codex adventure entry. Call `novel (action: create, "my-game", codex_adventure="dragon-hoard")` — assert Novel created, adventure scaffold imported (world-model populated, NPCs created, factions set, lore entries present, synthesis linkages active), `adventure_set: true` in metadata. Call `novel (action: create, "broken", codex_adventure="nonexistent")` — assert `[NOT_FOUND]`. Call `novel (action: create, "wrong-kind", codex_adventure="blacksmith")` where blacksmith is kind `npc` — assert `[NOT_FOUND]` (wrong kind). | REQ-088, REQ-321 |
| T380 | Automated | Story journal to lore promotion: call `story (action: record, "revelation", "The old well leads to the undercity")` — assert entry at index 0. Call `codex (action: list))` — assert lore entry `the-old-well-leads-to-the-undercity` created with `source: story_journal:0`. Call `story (action: list)` — assert journal entry unchanged (non-destructive). Call `story (action: record, "decision", "We chose to trust the vampire")` — assert at index 1. Call `codex (action: list))` — assert `[RULE_VIOLATION]`. Call `codex (action: list), key="well-undercity-link")` — assert `[STATE_CONFLICT]` (key already taken). Player badge — assert `[FORBIDDEN]`. | REQ-333 |
| T381 | Automated | Novel archiving: create a Novel with entities, NPCs, lore entries, and an active adventure. Call `novel (action: archive, "my-novel")` — assert file moved to `.holonovel-state/archive/my-novel.json`. Call `novel (action: list)` — assert Novel absent. Call `novel (action: list, filter="archived")` — assert Novel present with archive timestamp and metadata. Call `novel (action: resume, "my-novel")` — assert `[STATE_CONFLICT]`. Call `novel (action: info, "my-novel")` — assert read-only metadata returned. Call `novel (action: unarchive, "my-novel")` — assert file restored to `.holonovel-state/novels/my-novel.json`. Set base-capability state before archiving — assert `novel (action: unarchive)` restores it (REQ-434–443). Give an NPC a populated mind before archiving — assert `novel (action: unarchive)` restores it (REQ-075f). Call `novel (action: resume, "my-novel")` — assert full state intact. Call `novel (action: archive, "my-novel")` while another connection has it active — assert `[STATE_CONFLICT]`. Player badge `novel (action: archive)` — assert `[FORBIDDEN]`. `spec_health.archived_novels` — assert lists the archived slug. | REQ-334 |
| T382 | Automated | Batch codex import: call `codex (action: import, ["blacksmith", "innkeeper"])` — assert both NPCs created, single audit entry recorded, one undo snapshot restores both. Call `codex (action: import, ["existing-npc", "nonexistent"])` — assert `[NOT_FOUND]` for `nonexistent` at index 1; assert `existing-npc` was NOT created (atomic rollback). | REQ-321 |
| T383 | Automated | Bidirectional codex sync: call `codex (action: import, "blacksmith")` into Novel — assert NPC with `codex_source` created. Set personality and voice_examples on the NPC via `character (action: personality)` and `character (action: voice)`. Call `codex (action: capture, "npc", "blacksmith", update_source=true)` — assert Codex entry "blacksmith" updated in-place (personality, voice_examples reflected), NOT a new entry. Call `codex (action: get, "blacksmith")` — assert updated data payload. Call `codex (action: capture, "npc", "guard-captain", update_source=true)` where guard-captain has no codex_source — assert `[STATE_CONFLICT]`. | REQ-321, REQ-332 |
| T384 | Automated | Codex provenance: call `codex (action: import, "blacksmith")` into active Novel — assert NPC carries `codex_source: {id: "blacksmith", imported_at: <ISO>, codex_modified_at: <ISO>}`. Call `novel (action: info)` — assert `codex_sources` includes `{id: "blacksmith", kind: "npc"}`. Update blacksmith Codex entry via `codex (action: set)` (change description). Call `session (action: health)` — assert `[codex-stale]` flag for blacksmith. Call `codex (action: import, "blacksmith")` again — assert existing NPC updated (description changed, HP and conditions preserved), NOT duplicated. Call `novel (action: clone, ...)` — assert cloned NPC retains `codex_source`. | REQ-332, REQ-258, REQ-097 |
| T385 | Automated | Scene beat taxonomy: call `scene (action: set, "The hall darkens", beat="escalation")`. Invoke `badge_briefing` — assert scene state section includes `Beat: escalation` after scene type tag. Call `session (action: recap)` — assert `beat_transitions` array includes `{from: "mid_scene", to: "escalation"}`. Call `scene (action: set, "Still dark", beat="escalation")` — assert no new transition entry (same beat). Call `scene (action: set, "Dawn breaks", beat="resolution")` — assert second transition entry. Assert `badge_briefing` shows `Beat: resolution`. | REQ-335 |
| T386 | Automated | Dramatic pacing signal: set `TTRPG_PACING_WINDOW=3`. Call `scene (action: set, "Start", beat="setup")`. Perform 4 tool calls (any non-mutating tools). Invoke `badge_briefing` — assert `[pacing] Scene stabilized — 4 actions since last transition.` Call `scene (action: set, "New scene")` — assert counter reset, pacing signal absent. Set `TTRPG_PACING_WINDOW=0` — assert no pacing signal after 20+ calls. | REQ-336 |
| T387 | Automated | Narrative arc visibility: call `scene (action: set, "The hall is quiet", beat="setup")`, then `scene (action: set, "Torches flicker", beat="escalation")`, then `scene (action: set, "The door bursts open", beat="climax")`. Invoke `badge_briefing` under GM — assert `story_beats` line contains three-beat sequence with scene previews. Invoke under Player — assert only `shared`-scope beats visible. Perform 12 beat transitions — assert only most recent 10 appear. Empty Novel — assert `[No beats completed.]`. | REQ-337 |
| T388 | Automated | Faction autonomous advancement: create faction with `TTRPG_FACTION_AUTONOMY_INTERVAL=3`. Call `scene (action: set, "Scene A")` — assert no autonomous tick (interval not yet met). Call scene transitions for Scenes B and C — assert clock advances by 1 autonomous tick on 3rd transition with `[autonomous]` annotation. Advance through Scenes D, E, F — assert 2nd autonomous tick on 6th transition. Create linked countdown and fill via autonomous tick — assert `[pending-fire]` in `badge_briefing`. Set interval to zero — assert no autonomous ticks. | REQ-338 |
| T389 | Automated | NPC goal pursuit: create NPC with `goals="Steal the crown"`, `disposition="suspicious"`. Set `TTRPG_NPC_AUTONOMY=on`. Call `scene (action: set, "Throne room")` — assert `badge_briefing` World in Motion includes goal-pursuit suggestion. Accept suggestion — assert state change applies, suggestion absent on next transition. Defer — assert re-appears at next transition. Dismiss — assert does not re-appear. Set `TTRPG_NPC_AUTONOMY=off` — assert no suggestions. | REQ-339 |
...
| T392 | Automated | Scene from world model: call `scene (action: set, "", location="Throne Room")` with Throne Room containing throne thing, chandelier, and 2 NPCs. Assert scene description derived from room description + contents. Call `scene (action: set, "The royal chamber", location="Throne Room")` — assert explicit description used, not derived. Call `scene (action: set, "The void", location="Nowhere")` — assert description parameter used alone (no match). Call `undo` — assert scene state restored. | REQ-342 |
| T393 | Automated | Unified intent: call `command (action: suggest, "convince the guard to let us pass", entity_id="bard_01")` — assert results grouped by domain (mechanical, spatial, social). Assert social domain includes guard's disposition and relationship. Call `command (action: suggest, "attack the goblin")` — assert only mechanical suggestions. Call `command (action: suggest, "sneak past")` — assert mechanical + spatial. Call under Player badge — assert all three domains return. | REQ-343 |
| T394 | Automated | Voice feedback: call `character (action: signal, "voice_feedback", "She wouldn't say that — she'd say 'The door is trapped. Stand back.'")`. Assert `character (action: sheet)` shows `[player-corrected]` voice example. Assert `badge_briefing` shows correction. Assert audit log entry recorded. Call 4th correction in same session — assert `[WARNING] Voice correction limit reached for this session.`. Change `TTRPG_SESSION_ID` — assert limit reset. | REQ-344 |
...
| T397 | Automated | Voice codex capture: call `character (action: signal, "voice_feedback", "She wouldn't say that — she'd say 'Stand back.'")` on an entity. Call `codex (action: capture, "voice_profile", entity_id, update_source=true)` — assert Codex entry created with `kind: "voice_profile"`, `source_novel` populated, `original_text` and `corrected_text` pairs, and `[codex-corrected]` tag. Call `codex (action: list, "voice_profile")` — assert entry present. Call `codex (action: import, entry_id)` into a new Novel — assert matching entity's `voice_examples` includes the corrected snippet tagged `[codex-corrected]`. Call `codex (action: capture, "voice_profile", entity_id)` without `update_source` — assert new entry created (not overwritten). Assert `badge_briefing` renders `[codex-corrected]` voice examples visually distinct from `[player-corrected]`. | REQ-347, REQ-321 |
| T398 | Automated | Faction-NPC coordination: create faction "Merchant Guild" with goal "Expand to East Dock". Create NPC "Guildmaster Kael" with `goals="Secure the East Dock contract"` and `disposition="suspicious"`. Set `TTRPG_FACTION_AUTONOMY_INTERVAL=3`, `TTRPG_NPC_AUTONOMY=on`. Call `scene (action: set, "Scene A")`, then B, then C — assert autonomous tick fires on 3rd transition, faction clock advances, and Kael's goal pursuit suggestion is absent (suppressed). Assert `[faction-npc-coordination]` audit entry present. Call `scene (action: set, "Scene D")` — no faction tick; assert Kael's goal pursuit suggestion appears in `## World in Motion`. Set `TTRPG_NPC_AUTONOMY=off` — assert faction autonomous ticks proceed without NPC coordination. | REQ-348 |
...
| T401 | Automated | Pacing-triggered autonomy: set `TTRPG_PACING_WINDOW=3`. Create faction "Thieves Guild" with clock and NPC "Locke" with `goals="Crack the vault"`, `disposition="suspicious"`. Set `TTRPG_FACTION_AUTONOMY_INTERVAL=5`, `TTRPG_NPC_AUTONOMY=on`. Perform 4 tool calls without scene transition — assert pacing signal fires AND `[pacing-autonomy]` audit entry recorded with faction tick annotation AND Locke's goal pursuit suggestion appears in `## World in Motion`. Perform 4 more tool calls — assert pacing signal re-fires but `[pacing-autonomy]` does NOT re-trigger. Call `scene (action: set, "New scene")` — assert counter resets. Perform 4 more calls — assert `[pacing-autonomy]` fires again. Create overlapping faction+NPC goal (NPC whose goal text intersects faction goal text) — assert REQ-348 suppression applies during pacing autonomy. | REQ-351, REQ-336, REQ-338, REQ-339 |
| T402 | Automated | Codex adventure beat sequences: create Codex adventure entry via `codex (action: set, "adventure", "Test Quest", {title: "Test Quest", premise: "...", sections: {}, suggested_beats: [{beat: "setup", scene_preview: "The tavern is quiet..."}, {beat: "escalation", scene_preview: "A fight breaks out..."}, {beat: "climax", scene_preview: "The dragon rises!"}]})`. Call `novel (action: create, "test-quest", codex_adventure="test-quest")` — assert `badge_briefing` `story_beats` shows all three beats tagged `[adventure-scaffold]`. Call `scene (action: set, "The tavern hums with conversation", beat="setup")` — assert first beat replaced, `[adventure-scaffold]` tag removed on this entry. Advance scene to second beat without explicit `beat` parameter — assert second scaffold beat still present with `[adventure-scaffold]` tag. Advance to third beat via `scene (action: set, beat="climax")` — assert scaffold replaced. Create adventure WITHOUT `suggested_beats` — call `codex (action: import)` — assert no beat pre-population. | REQ-352, REQ-088, REQ-321, REQ-337 |
...
| T414 | Automated | Secret-world coupling: create world-model room "Vault". Call `lore (action: set_secret, "vault-trap", "The floor is pressure-plated", world_target="vault")`. Call `scene (action: set, "The strongroom", location="Vault")` — assert `badge_briefing` `narrative_threads` includes `[world-linked]` vault-trap entry. Call `scene (action: set, ..., location="Inn")` — assert entry absent. | REQ-363, REQ-234, REQ-195 |
| T415 | Automated | Faction-world coupling: create room "Throne Room". Call `faction (action: create, "Royal Guard", goals=["Protect the crown"], territory=["throne_room"])`. Call `scene (action: set, ..., location="Throne Room")` — assert `badge_briefing` `narrative_threads` includes `[territorial] Royal Guard`. Call `scene (action: set, ..., location="Pantry")` — assert faction absent. | REQ-364, REQ-233, REQ-195 |
| T416 | Automated | Server notes narrative coupling: call `note (action: set_server, "old-gods", "The old gods were banished...", narrative_tag="lore_seed")` — assert `badge_briefing` supplementary guidance includes `[lore-seed]`. Call `note (action: set_server, "dm-reminder", "Remind players...", narrative_tag="session_reminder")` — assert `[session-reminder]` tag. Call without `narrative_tag` — assert absent from briefing. Player badge — assert absent. | REQ-365, REQ-285 |
| T417 | Automated | Observer narrative surface: call `set_badge("observer")` on a populated Novel with entities carrying both active and non-present states, and knowledge gating active (entity present in some scenes, absent from others). Assert `badge_briefing` includes scene state, entity personality, and narrative threads with omniscient-role orientation directive. Assert `badge_briefing` includes `[not present]` markers for entities absent from the current scene. Assert `badge_briefing` includes `knowledge_state` for all entities in the Novel (not only the active entity), matching the GM-level visibility contract. Assert `badge_briefing` excludes secrets, faction clocks, countdown positions, and GM context. Assert `scene (action: set, ...)` from observer returns `[FORBIDDEN]`. | REQ-366, REQ-305 |
...
| T428 | Automated | Wisdom mechanical enactment Pattern Buffer: create NPC with host-carried Wisdom active — assert character (action: sheet) shows auto-populated voice_examples, goals, personality (P6). Create countdown — assert auto-advances on scene (action: set) (P7). command (action: suggest) returns constraint overrides (P10). Deactivate Wisdom items — assert behavior stops. Reactivate — assert resumes. Assert REQ-371 conformance: first-class mechanics, not advisory. | §6.6 S33, REQ-371 |
...
| T436 | Automated | Scene ↔ NPC couplings: create NPC with disposition=friendly. Call `scene (action: set, "...", scene_type="combat")` — assert `narrative_threads` includes NPC disposition advisory (P41). Call `scene (action: set, "...", scene_type="social")` — assert advisory updated. Create NPC with `location` matching active room — call `scene (action: set, "...", characters_present=["<entity>"])` — assert NPC auto-registers in scene per P42. | REQ-369, REQ-075 |
...
| T441 | Automated | Novel ruleset binding: call `novel (action: create, "test", ruleset="dnd5e")` — assert `ruleset: "dnd5e"` in `novel (action: info)`. Call `novel (action: create, "test2", ruleset="unknown")` — assert `[ERROR] [INVALID_INPUT]` with valid rulesets enumerated. Export and verify `ruleset` field in manifest. | REQ-380 |
...
| T446 | Automated | command (action: suggest) scoping: call `command (action: suggest, "attack")` under D&D Novel — assert D&D-prefixed tool suggestions only. Same intent under Starfinder Novel — assert Starfinder-prefixed only. | REQ-385 |
...
| T450 | Automated | Holodeck config discovery: build a server with TTRPG_PACING_WINDOW=6, TTRPG_NPC_AUTONOMY=off, TTRPG_WORLD_REACTIVITY=on. Call session (action: health) — assert holodeck_config.behavioral_coupled counts every behavioral variable, natural_language_paths includes pacing_window → "character (action: signal, pace, faster/slower)", npc_autonomy → "scene (action: directive, 'NPCs act independently')", npc_mind → "scene (action: directive, 'NPCs think for themselves')", world_reactivity → "scene (action: directive, 'the world reacts')", story_beat_window → "scene (action: directive, 'keep more beats')", campaign_memory_max_facts → "scene (action: directive, 'more campaign notes')", auto_record → "scene (action: directive, 'auto-record moments')", max_available_actions → "scene (action: directive, 'more options')", narration_validation → "scene (action: directive, 'validate my narration')", state_gate → "scene (action: directive, 'warn on state drift')". Assert uncoupled is empty. Assert system variables (TTRPG_MAX_NPCS, TTRPG_DATA_DIR) and build-time/presentation settings (TTRPG_WORLD_PROMINENCE, TTRPG_NOVEL_PREVIEW_CHARS) absent from behavioral counts. | REQ-388, REQ-069, REQ-081 |
...
`adventure (action: list)` (REQ-292). When `## Preview` is absent, `adventure (action: list)` SHALL derive
...
1. `scene (action: set, "The king's burial chamber", location="Throne Room")` — the
...
2. `command (action: resolve, "look")` from the Game Master badge returns:
   - Room: Throne Room
...
3. `command (action: resolve, "go south")` — spatial transition resolves correctly.
...
Ghost, Throne Room)` places the NPC in the Throne Room, visible on `command (action: resolve)`
from that room.
...
is populated when `adventure (action: load)` is called. TTRPG annotations are linked to
world-model objects by name; unmatched annotations are reported as unresolved
references. Adventure content appears in `ruleset (action: search)` results filtered by active
adventure and badge. The `adventure (action: load)` tool (REQ-079, REQ-292) sets the active adventure
...
`lore (action: export, markdown) → lore (action: import)` produces lore entries indistinguishable
from the `lore (action: export, json) → lore (action: import)` round-trip. Non-lore entries
...
**Round-trip fidelity.** `novel (action: export, format)` → `novel (action: import, data, mode)` →
`novel (action: export, format)` produces identical output for the same format. The builder
