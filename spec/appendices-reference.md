...
| T278  | Automated | Clone Novel: call `novel (action: clone, "my-novel", "my-novel-fork")` — assert new Novel created at `novels/my-novel-fork.json`, `spec_health` lists both Novels, source Novel's active flag unchanged. Mutate the clone (add NPC) — assert source Novel unaffected. Call `novel (action: clone, "my-novel", "my-novel-fork")` again — assert `[STATE_CONFLICT]`. Call `novel (action: clone, "my-novel", "trimmed", trim_audit_sessions=2)` — assert cloned audit log contains only 2 most recent sessions. Player badge `novel (action: clone)` returns `[FORBIDDEN]`. | REQ-240 |
| T279  | Automated | Checkpoints: call `novel (action: checkpoint_set, "before-ritual")` — assert checkpoint created, `novel (action: checkpoint_list)` returns `{label: "before-ritual", ...}`. Perform 5 mutations. Call `novel (action: checkpoint_restore, "before-ritual")` — assert `[NEED_INPUT]`, confirm `yes`, assert all 5 mutations reversed. Call `novel (action: checkpoint_remove, "before-ritual")` — assert `novel (action: checkpoint_list)` is empty. Set `TTRPG_MAX_CHECKPOINTS=1`, create two checkpoints — assert oldest discarded. Call `novel (action: end)` — assert checkpoints cleared. `novel (action: export, "json", include_checkpoints=true)` — assert `checkpoints` key present. Player badge returns `[FORBIDDEN]`. | REQ-241 |
