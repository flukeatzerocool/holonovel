...
| T158  | Automated | End-Novel confirmation dispatch: `manage_novel (action: end)` → assert `[NEED_INPUT]` with yes/cancel options. `respond("End Novel <slug>?", "yes")` → assert Novel file removed, active set empty. `manage_novel (action: resume, slug)` → assert `[STATE_CONFLICT]`. `respond` with non-matching decision → assert `[NOT_FOUND]` with open decision text. | REQ-140 |
...
| T225  | Automated | Section token discoverability: invoke `manage_session (action: health)` — assert `section_tokens` array with token, group, and has_content fields. Invoke `help` with query `"briefing"` — assert valid token set enumerated. Invoke `help` with query `"section ordering"` — assert valid token set enumerated. Invoke `manage_session (action: briefing_order)` with an unknown token — assert `[INVALID_INPUT]` with valid tokens enumerated matching `spec_health.section_tokens` exactly. | REQ-186, REQ-082 |
...
| T331 | Automated | Scene-state ledger: call `manage_scene (action: set, "Tavern", "Merry tavern")`. Assert badge_briefing includes scene token with current location and description. Call `manage_scene (action: set, "Forest", "Dark forest")` — assert prior scene pushed to ledger. Call `undo` — assert ledger restored to prior scene. Assert `manage_session (action: recap)` includes scene transitions. | REQ-076 |
...
| T392 | Automated | Scene from world model: call `manage_scene (action: set, "", location="Throne Room")` with Throne Room containing throne thing, chandelier, and 2 NPCs. Assert scene description derived from room description + contents. Call `manage_scene (action: set, "The royal chamber", location="Throne Room")` — assert explicit description used, not derived. Call `manage_scene (action: set, "The void", location="Nowhere")` — assert description parameter used alone (no match). Call `undo` — assert scene state restored. | REQ-342 |
